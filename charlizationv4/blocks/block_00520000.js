// ═══════════════════════════════════════════════════════════════════
// block_00520000.js — Mechanical transpilation of block_00520000.c
//
// IMPORTANT: Function names in comments are BEST GUESSES and may be
// wrong. Do not trust them blindly — verify against the decompiled C
// in reverse_engineering/decompiled/block_00520000.c if in doubt.
//
// Source: reverse_engineering/decompiled/block_00520000.c
// ═══════════════════════════════════════════════════════════════════




// ============================================================
// Function: FUN_00521807 @ 0x00521807
// Size: 12 bytes
// ============================================================

// CRT cleanup thunk
import { G } from '../globals.js';
import { s8, u8, s16, u16, s32, u32, w16, w32, getTileOffset, tileRead, tileWrite, initMapTiles } from '../mem.js';
import { FUN_004087c0, FUN_005ae052, FUN_005b8931, FUN_005b94d5, FUN_005b89bb, FUN_005b89e4, FUN_005b8a1d, FUN_005b8ca6, FUN_005b8ee1, FUN_004bd9f0, FUN_0058c56c, FUN_005b68f6 } from '../fn_utils.js';

export function FUN_00521807() {
  FUN_0059df8a();
  return;
}



// ============================================================
// Function: FUN_0052181d @ 0x0052181D
// Size: 15 bytes
// ============================================================

// SEH epilog (no-op in JS)
export function FUN_0052181d() {
  // SEH exception handler restoration — no-op in JS
  return;
}



// ============================================================
// Function: FUN_0052182c @ 0x0052182C
// Size: 159 bytes
// ============================================================

// mp_build_label_string
export function FUN_0052182c(param_1, param_2) {
  let sVar1;
  let uVar2;
  let local_84 = '';

  local_84 = '';
  FUN_004af14b(local_84, param_2);
  if (param_1[0x134] !== 0) {
    FUN_005f22e0(local_84, G.DAT_00632268);
    FUN_005f22e0(local_84, param_1[0x134]);
  }
  sVar1 = local_84.length; // _strlen
  uVar2 = FUN_00498159(param_1[0x254], sVar1 + 1);
  param_1[0x134] = uVar2;
  FUN_005f22d0(param_1[0x134], local_84);
  return;
}



// ============================================================
// Function: FUN_005218cb @ 0x005218CB
// Size: 1764 bytes
// ============================================================

// mp_scenario_load_dialog
export function FUN_005218cb(param_1) {
  let iVar1;
  let uVar2;
  let local_844 = new Array(1076);
  let local_410 = new Array(40);
  let local_3e8;
  let local_3d4;
  let local_11c;
  let local_118 = new Array(260);
  let local_14;

  FUN_0059db08(0x4000);
  FUN_005c64da();

  // LAB_00521912 — original goto target, restructured as labeled loop
  LAB_00521912:
  while (true) {
    do {
      if (G.DAT_0062b420 === 0) {
        FUN_0046e6a9();
        FUN_0046e020(0x6b, 0, 1, 0);
      }
      FUN_005bf5e1(0x5a, 10, 0xc0, local_844);
      FUN_00419be0(G.DAT_0063cbd0);
      FUN_00419ba0(0x9e);
      FUN_00419b80();
      iVar1 = FUN_load_verify_units(1, param_1, 1);
      if (iVar1 !== 0) {
        FUN_0046e020(0x6a, 0, 0, 0);
        FUN_005bf5e1(0x5a, 10, 0xc0, local_844);
        FUN_00419be0(G.DAT_0063cbd0);
        FUN_00419ba0(0x9e);
        FUN_00419b80();
        FUN_00521faf();
        FUN_00521fbb();
        FUN_00521fd1();
        return;
      }
      FUN_004729ab(local_118, G.DAT_00679640, G.DAT_0062cd24);
      if (G.DAT_0062b420 !== 0) {
        FUN_0046e020(0x6a, 0, 0, 0);
      }
      G.DAT_00655aea = G.DAT_0064bc1e & 0xffff7fff;
      G.DAT_00655af2 = G.DAT_0064bc22;
      FUN_0040ff60(0, G.DAT_0064bc62);
      FUN_00421da0(0, G.DAT_0064bcb2 | 0);
      FUN_0040bbb0();
      uVar2 = FUN_00484fec(G.DAT_00655af8 | 0);
      FUN_00421f10(uVar2);
      FUN_0040ff60(1, G.DAT_00679640);
      FUN_0040bbb0();
      if (G.DAT_0064bcb8 === 0) {
        FUN_00421f10(2000);
      }
      else {
        uVar2 = FUN_00484fec(G.DAT_0064bcb8 | 0);
        FUN_00421f10(uVar2);
      }
      FUN_0040ff60(2, G.DAT_00679640);
      if (G.DAT_0062b420 !== 0) {
        FUN_00419c8b();
      }
      FUN_00421dd0();
      iVar1 = CSocket_Create(local_410, local_118, 0x63226c, 0x4008001);
      if (iVar1 !== 0) {
        // goto LAB_00521beb — handled below
        iVar1 = FUN_00421e70(s_SCENARIOLOADED_00632278, 1);
        if (iVar1 >= 0) {
          // goto LAB_00521c07 — fall through to outer while(true)
          break; // break out of do-while
        }
        continue LAB_00521912; // goto LAB_00521912
      }
      iVar1 = G.DAT_00655afe | 0;
      if (((iVar1 >= 0) && (iVar1 < G.DAT_00655b16)) && (G.DAT_0065610a[iVar1 * 0x20] !== 0)
         && (s8(G.DAT_006560f7[iVar1 * 0x20]) === (G.DAT_0064bcba | 0))) {
        FUN_0059ec88(G.DAT_00641848 + u8(G.DAT_006560f6[iVar1 * 0x20]) * 0x3c, 0, 0);
      }
      iVar1 = FUN_0040bc80(0);
    } while (iVar1 < 0);
    // goto LAB_00521c07 — fall through

    // LAB_00521c07
    while (true) {
      FUN_0040ffa0(s_DIFFICULTY_00632288, 1);
      FUN_0059ea99(G.DAT_00655b08);
      FUN_0059e783(((((G.DAT_006ab198 + -0x280 + (G.DAT_006ab198 + -0x280 >> 0x1f & 7)) >>> 0) >> 3) | 0)
                   + 1, -((((G.DAT_006ab19c + -0x1e0 + (G.DAT_006ab19c + -0x1e0 >> 0x1f & 7)) >>> 0) >> 3 | 0)
                         + 1));
      local_14 = FUN_0040bc80(0);
      if (local_14 < 0) break;
      G.DAT_00655b08 = u8(local_14);
      if (u8(local_14) === 0) {
        G.DAT_00655aea = G.DAT_00655aea | 0x100;
      }
      else {
        G.DAT_00655aea = G.DAT_00655aea & 0xfffffeff;
      }
      while (true) {
        G.DAT_00654fa6 = 0;
        G.DAT_0062c488 = 0;
        FUN_0051d7bc();
        FUN_0051d7d6(0, G.DAT_00666546 | 0);
        FUN_0051d7d6(1, G.DAT_00666548 | 0);
        FUN_0051d7d6(2, G.DAT_0066654a | 0);
        FUN_0051d7d6(3, G.DAT_0066654c | 0);
        FUN_0051d7d6(4, G.DAT_006665fe | 0);
        FUN_0051d7d6(5, G.DAT_00666600 | 0);
        FUN_005a632a(G.DAT_006359d4, s_ADVANCEDMP_00632294, 0, 0, 0, 0, 0, 0);
        if (G.DAT_006c3160 === 0) {
          G.DAT_006ad684 = 0;
          G.DAT_00654c7c = 0;
        }
        else {
          FUN_0040bbb0();
          FUN_0040bc10(0x364);
          FUN_0059edf0(G.DAT_00679640, 6, 0);
          FUN_0051d7d6(6, G.DAT_0066654e | 0);
        }
        if ((local_3d4 & 4) !== 0) {
          for (local_11c = 0; local_11c < local_3e8; local_11c = local_11c + 1) {
            FUN_0059ea4d(local_11c, 1 << (u8(local_11c) & 0x1f) & G.DAT_00631ed8);
          }
        }
        FUN_0059e783(((((G.DAT_006ab198 + -0x280 + (G.DAT_006ab198 + -0x280 >> 0x1f & 7)) >>> 0) >> 3) | 0)
                     + 1, -((((G.DAT_006ab19c + -0x1e0 + (G.DAT_006ab19c + -0x1e0 >> 0x1f & 7)) >>> 0) >> 3 | 0)
                           + 1));
        iVar1 = FUN_0040bc80(0);
        FUN_0046e020(0x6a, 0, 0, 0);
        if (iVar1 < 0) break;
        G.DAT_00654c74 = FUN_0059e9f3(0);
        G.DAT_00666546 = G.DAT_00654c74;
        G.DAT_00654c76 = FUN_0059e9f3(1);
        G.DAT_00666548 = G.DAT_00654c76;
        G.DAT_00654c78 = FUN_0059e9f3(2);
        G.DAT_0066654a = G.DAT_00654c78;
        G.DAT_00654c7a = FUN_0059e9f3(3);
        G.DAT_0066654c = G.DAT_00654c7a;
        G.DAT_00654fac = FUN_0059e9f3(4);
        G.DAT_006665fe = G.DAT_00654fac;
        G.DAT_00654fae = FUN_0059e9f3(5);
        G.DAT_00666600 = G.DAT_00654fae;
        if (G.DAT_006c3160 !== 0) {
          G.DAT_00654c7c = FUN_0059e9f3(6);
          G.DAT_0066654e = G.DAT_00654c7c;
        }
        if ((param_1 === 2) || (iVar1 = FUN_0051ea8e(0), iVar1 === 0)) {
          FUN_00521faf();
          FUN_00521fbb();
          FUN_00521fd1();
          return;
        }
      }
    }
    // goto LAB_00521912 — continue outer loop
    continue LAB_00521912;
  }
}



// ============================================================
// Function: FUN_00521faf @ 0x00521FAF
// Size: 12 bytes
// ============================================================

// CRT cleanup thunk (FUN_005c656b)
export function FUN_00521faf() {
  FUN_005c656b();
  return;
}



// ============================================================
// Function: FUN_00521fbb @ 0x00521FBB
// Size: 12 bytes
// ============================================================

// CRT destructor thunk
export function FUN_00521fbb() {
  FUN_0059df8a();
  return;
}



// ============================================================
// Function: FUN_00521fd1 @ 0x00521FD1
// Size: 15 bytes
// ============================================================

// SEH epilog (no-op in JS)
export function FUN_00521fd1() {
  // SEH exception handler restoration — no-op in JS
  return;
}



// ============================================================
// Function: FUN_00521fe0 @ 0x00521FE0
// Size: 1591 bytes
// ============================================================

// mp_choose_players_dialog
export function FUN_00521fe0(param_1) {
  let uVar1;
  let iVar2;
  let local_444;
  let local_440;
  let local_43c;
  let local_438 = new Array(260);
  let local_334;
  let local_330;
  let local_32c = new Array(756);
  let local_38;
  let local_34;
  let aiStack_30 = new Array(8);

  FUN_0059db08(0x4000);
  G.DAT_00655b0b_val = 0;
  G.DAT_00628048 = 0;
  local_330 = 0;
  do {
    if (param_1 <= local_330) {
      G.DAT_00654fa4 = G.DAT_00628048;
      G.DAT_006d1da0 = G.DAT_00628048 >>> 0;
      G.DAT_00655b05 = G.DAT_00628048;
      G.DAT_00654fa6 = 1;
      G.DAT_0062c488 = 1;
      FUN_00522617();
      FUN_0052262d();
      return;
    }
    while (true) {
      while (true) {
        do {
          FUN_0040bc40(1);
          FUN_0052182c(local_32c, local_330 + 0x25e);
          local_334 = 0;
          for (local_444 = 1; local_444 < 8; local_444 = local_444 + 1) {
            if ((1 << (u8(local_444) & 0x1f) & (G.DAT_00655b0a >>> 0)) !== 0) {
              FUN_0040bbb0();
              uVar1 = FUN_00493c7d(local_444);
              FUN_0040bbe0(uVar1);
              FUN_0040fe10();
              FUN_0040fea0();
              uVar1 = FUN_00493b10(local_444);
              FUN_0040bbe0(uVar1);
              FUN_0040fed0();
              FUN_0040fe10();
              FUN_0040fea0();
              FUN_0040ff30(G.DAT_0064c708[local_444 * 0x594] | 0);
              FUN_0040fe10();
              FUN_0040bc10(0xc5);
              FUN_0040bbe0(G.DAT_006322a0);
              FUN_0040ff30(G.DAT_0064c706[local_444 * 0x594] | 0);
              FUN_0040fe10();
              FUN_0040bc10(0xc4);
              FUN_0040bbe0(G.DAT_006322a4);
              local_38 = FUN_0059edf0(G.DAT_00679640, local_444, 0);
              aiStack_30[local_38[4]] = local_444;
              if ((1 << (u8(local_444) & 0x1f) & (G.DAT_00655b0b_val >>> 0)) === 0) {
                if (local_334 === 0) {
                  local_334 = 1;
                  FUN_0059ea99(local_38[4]);
                }
              }
              else {
                FUN_0059e8db(local_38[4], 1);
              }
            }
          }
          iVar2 = FUN_0040bc80(0);
          if (iVar2 < 0) {
            FUN_00522617();
            FUN_0052262d();
            return;
          }
          G.DAT_006d1da0 = aiStack_30[iVar2];
          local_444 = u8(G.DAT_006d1da0);
          if (local_330 === 0) {
            G.DAT_00628048 = u8(local_444);
          }
          G.DAT_00655b0b_val = G.DAT_00655b0b_val | u8(1 << (u8(local_444) & 0x1f));
          G.DAT_00655b03 = u8(local_444);
          G.DAT_00655b05 = u8(local_444);
          G.DAT_00655aea = G.DAT_00655aea & 0xfffffeff;
          iVar2 = FUN_005a632a(G.DAT_006359d4, G.DAT_006322a8, 0x17,
                               G.DAT_0064bcfa + s8(u8(local_444)) * 0xf2, 0, 0, 0, 1);
        } while (iVar2 !== 0);
        FUN_0052182c(local_32c, local_330 + 0x25e);
        iVar2 = FUN_005a5f34(local_438, 0);
        if (iVar2 === 0) break;
        G.DAT_00655b0b_val = G.DAT_00655b0b_val & ~u8(1 << (u8(local_444) & 0x1f));
      }
      FUN_005f22d0(G.DAT_0064bcfa + s8(G.DAT_00655b03) * 0xf2, local_438);
      FUN_0040ffa0(s_GENDER_006322b0, 1);
      FUN_0052182c(local_32c, local_330 + 0x25e);
      FUN_0059ea99(G.DAT_006554fc[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30]);
      FUN_0059e783(0xfffffc19,
                   -((((G.DAT_006ab19c + -0x1e0 + (G.DAT_006ab19c + -0x1e0 >> 0x1f & 7)) >>> 0) >> 3 | 0)
                     + 1));
      iVar2 = FUN_0040bc80(0);
      FUN_0046e020(0x6a, 0, 0, 0);
      if (iVar2 >= 0) break;
      G.DAT_00655b0b_val = G.DAT_00655b0b_val & ~u8(1 << (u8(local_444) & 0x1f));
    }
    local_440 = u8(iVar2);
    G.DAT_006554fc[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30] = local_440;
    G.DAT_0064c6a0[G.DAT_006d1da0 * 0x594] =
         G.DAT_0064c6a0[G.DAT_006d1da0 * 0x594] & 0xfdff;
    if (iVar2 !== 0) {
      G.DAT_0064c6a0[G.DAT_006d1da0 * 0x594] =
           G.DAT_0064c6a0[G.DAT_006d1da0 * 0x594] | 0x200;
    }
    for (local_34 = 0; local_34 < 7; local_34 = local_34 + 1) {
      for (local_43c = 0; local_43c < 2; local_43c = local_43c + 1) {
        G.DAT_0065550c[local_34 * 4 +
                  G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30 + local_43c * 2] =
             G.DAT_00654fe0[local_34 * 8 + local_43c * 4] | 0;
      }
    }
    local_330 = local_330 + 1;
  } while (true);
}



// ============================================================
// Function: FUN_00522617 @ 0x00522617
// Size: 12 bytes
// ============================================================

// CRT destructor thunk
export function FUN_00522617() {
  FUN_0059df8a();
  return;
}



// ============================================================
// Function: FUN_0052262d @ 0x0052262D
// Size: 15 bytes
// ============================================================

// SEH epilog (no-op in JS)
export function FUN_0052262d() {
  // SEH exception handler restoration — no-op in JS
  return;
}



// ============================================================
// Function: FUN_0052263c @ 0x0052263C
// Size: 185 bytes
// ============================================================

// mp_set_animation_style
export function FUN_0052263c(param_1, param_2) {
  if (param_2 !== 0) {
    G.DAT_00631ef0 = 5;
  }
  if (param_1 === -1) {
    if (G.DAT_00631ef0 === 5) {
      G.DAT_00631ef0 = 7;
    }
    else if (G.DAT_00631ef0 === 6) {
      G.DAT_00631ef0 = 5;
    }
    else if (G.DAT_00631ef0 === 7) {
      G.DAT_00631ef0 = 6;
    }
    FUN_0055a41d(2, 6, G.DAT_00631ef0);
  }
  else {
    FUN_0055a41d(2, 6, param_1);
  }
  return;
}



// ============================================================
// Function: FUN_005226fa @ 0x005226FA
// Size: 192 bytes
// ============================================================

// mp_get_scroll_offset
export function FUN_005226fa(param_1) {
  let iVar1;

  if (param_1 !== 0) {
    G.DAT_00631eec = 1;
  }
  switch (G.DAT_00631eec) {
  case 1:
    G.DAT_00631eec = 3;
    iVar1 = -999;
    break;
  case 2:
    G.DAT_00631eec = 1;
    iVar1 = ((((G.DAT_006ab198 + -0x280 + (G.DAT_006ab198 + -0x280 >> 0x1f & 7)) >>> 0) >> 3) | 0) + 1;
    break;
  case 3:
    G.DAT_00631eec = 4;
    iVar1 = -(((((G.DAT_006ab198 + -0x280 + (G.DAT_006ab198 + -0x280 >> 0x1f & 7)) >>> 0) >> 3) | 0) + 1);
    break;
  case 4:
    G.DAT_00631eec = 2;
    iVar1 = -999;
    break;
  default:
    iVar1 = -999;
  }
  return iVar1;
}



// ============================================================
// Function: FUN_005227e3 @ 0x005227E3
// Size: 773 bytes
// ============================================================

// mp_hotseat_config_dialog
export function FUN_005227e3() {
  let iVar1;
  let local_30c;

  FUN_0059db08(0x4000);
  if (G.DAT_00655b02 < 3) {
    FUN_0052263c(0xffffffff, 0);
    FUN_0040ffa0(s_HOTSEAT2_006322b8, 1);
    if (4 < G.DAT_006c3164) {
      FUN_0040bbb0();
      FUN_0040bc10(0x365);
      FUN_0059edf0(G.DAT_00679640, 3, 0);
    }
    if (5 < G.DAT_006c3164) {
      FUN_0040bbb0();
      FUN_0040bc10(0x366);
      FUN_0059edf0(G.DAT_00679640, 4, 0);
    }
    if (6 < G.DAT_006c3164) {
      FUN_0040bbb0();
      FUN_0040bc10(0x367);
      FUN_0059edf0(G.DAT_00679640, 5, 0);
    }
    FUN_0059ea99(G.DAT_006665d8 | 0);
    FUN_0059e783(-(((((G.DAT_006ab198 + -0x280 + (G.DAT_006ab198 + -0x280 >> 0x1f & 7)) >>> 0) >> 3) | 0)
                  + 1), -((((G.DAT_006ab19c + -0x1e0 + (G.DAT_006ab19c + -0x1e0 >> 0x1f & 7)) >>> 0) >> 3 | 0)
                         + 1));
    // NOTE: C has fall-through switch cases here
    switch (G.DAT_00655b0d) {
    case 1:
      FUN_0059e8db(1, 1);
      // fall through
    case 2:
      FUN_0059e8db(2, 1);
      // fall through
    case 3:
      if (4 < G.DAT_006c3164) {
        FUN_0059e8db(3, 1);
      }
      // fall through
    case 4:
      if (5 < G.DAT_006c3164) {
        FUN_0059e8db(4, 1);
      }
      // fall through
    case 5:
      if (6 < G.DAT_006c3164) {
        FUN_0059e8db(5, 1);
      }
    }
    if (G.DAT_006665d8 + 1 < (G.DAT_00655b0d >>> 0) | 0) {
      FUN_0059ea99(G.DAT_006665d8 | 0);
    }
    iVar1 = FUN_0040bc80(0);
    if (iVar1 < 0) {
      FUN_00522b06();
      FUN_00522b1c();
      return;
    }
    G.DAT_006665d8 = iVar1 & 0xffff;
    FUN_0046e020(0x6a, 0, 0, 0);
    FUN_0055a567();
  }
  else {
    for (local_30c = 1; local_30c < 8; local_30c = local_30c + 1) {
      // empty loop body — original C has no body
    }
  }
  FUN_00522b06();
  FUN_00522b1c();
  return;
}



// ============================================================
// Function: FUN_00522b06 @ 0x00522B06
// Size: 12 bytes
// ============================================================

// CRT destructor thunk
export function FUN_00522b06() {
  FUN_0059df8a();
  return;
}



// ============================================================
// Function: FUN_00522b1c @ 0x00522B1C
// Size: 15 bytes
// ============================================================

// SEH epilog (no-op in JS)
export function FUN_00522b1c() {
  // SEH exception handler restoration — no-op in JS
  return;
}



// ============================================================
// Function: FUN_00522b2b @ 0x00522B2B
// Size: 683 bytes
// ============================================================

// mp_join_game_handler
export function FUN_00522b2b() {
  let uVar1;
  let uVar2;
  let uVar3;
  let iVar4;
  let uVar5;
  let local_324;
  let local_320;
  let local_1c;
  let local_14;

  FUN_0059db08(0x4000);
  uVar3 = G.DAT_006d1da0;
  uVar2 = G.DAT_00655b0b_val;
  uVar1 = G.DAT_00655b03;
  G.DAT_00655b0b_val = G.DAT_006c31a9;
  FUN_0055ae80(1);
  FUN_citywin_994F();
  FUN_004503d0();
  FUN_004503d0();
  for (local_14 = 0; local_14 < 8; local_14 = local_14 + 1) {
    if (G.DAT_0066ca84[local_14 * 0x3f0] !== 0) {
      FUN_004503d0();
    }
  }
  FUN_00419b80();
  local_1c = 0;
  local_324 = 0;
  for (local_320 = 1; local_320 < 8; local_320 = local_320 + 1) {
    if ((1 << (u8(local_320) & 0x1f) & (G.DAT_00655b0a >>> 0)) !== 0) {
      if ((1 << (u8(local_320) & 0x1f) & (G.DAT_00655b0b_val >>> 0)) === 0) {
        local_1c = local_1c + 1;
      }
      else {
        local_324 = local_324 + 1;
      }
    }
  }
  if (G.DAT_006c3164 === local_324) {
    FUN_00421da0(0, G.DAT_006c3164);
    FUN_00421ea0(s_JOINGAME1_006322c4);
  }
  else if (local_1c === 0) {
    FUN_00421ea0(s_JOINGAME0_006322d0);
  }
  else {
    iVar4 = FUN_00523f02(local_324 + -1);
    if (iVar4 === 0) {
      uVar5 = FUN_00493c7d(G.DAT_006d1da0);
      FUN_00421d60(0, uVar5);
      FUN_00421ea0(s_JOINGAME3_006322dc);
    }
  }
  G.DAT_006c31a9 = G.DAT_00655b0b_val;
  G.DAT_00655b03 = uVar1;
  G.DAT_00655b0b_val = uVar2;
  G.DAT_006d1da0 = uVar3;
  FUN_004085f0();
  FUN_004085f0();
  for (local_14 = 0; local_14 < 8; local_14 = local_14 + 1) {
    if (G.DAT_0066ca84[local_14 * 0x3f0] !== 0) {
      FUN_004085f0();
    }
  }
  FUN_0055b046(1);
  FUN_00522dd6();
  FUN_00522dec();
  return;
}



// ============================================================
// Function: FUN_00522dd6 @ 0x00522DD6
// Size: 12 bytes
// ============================================================

// CRT destructor thunk
export function FUN_00522dd6() {
  FUN_0059df8a();
  return;
}



// ============================================================
// Function: FUN_00522dec @ 0x00522DEC
// Size: 14 bytes
// ============================================================

// SEH epilog (no-op in JS)
export function FUN_00522dec() {
  // SEH exception handler restoration — no-op in JS
  return;
}



// ============================================================
// Function: FUN_00522dfa @ 0x00522DFA
// Size: 405 bytes
// ============================================================

// mp_negate_leader_ids
export function FUN_00522dfa() {
  let local_c;
  let local_8;

  for (local_c = 0; local_c < 0x15; local_c = local_c + 1) {
    if (G.DAT_00655502[local_c * 0x30] < 1) {
      G.DAT_00655502[local_c * 0x30] = (~G.DAT_00655502[local_c * 0x30] + 1) & 0xffff;
    }
    if (G.DAT_00655504[local_c * 0x30] < 1) {
      G.DAT_00655504[local_c * 0x30] = (~G.DAT_00655504[local_c * 0x30] + 1) & 0xffff;
    }
    if (G.DAT_00655506[local_c * 0x30] < 1) {
      G.DAT_00655506[local_c * 0x30] = (~G.DAT_00655506[local_c * 0x30] + 1) & 0xffff;
    }
    for (local_8 = 0; local_8 < 7; local_8 = local_8 + 1) {
      if (G.DAT_0065550c[local_8 * 4 + local_c * 0x30] < 1) {
        G.DAT_0065550c[local_8 * 4 + local_c * 0x30] =
             (~G.DAT_0065550c[local_8 * 4 + local_c * 0x30] + 1) & 0xffff;
      }
      if (G.DAT_0065550e[local_8 * 4 + local_c * 0x30] < 1) {
        G.DAT_0065550e[local_8 * 4 + local_c * 0x30] =
             (~G.DAT_0065550e[local_8 * 4 + local_c * 0x30] + 1) & 0xffff;
      }
    }
  }
  return;
}



// ============================================================
// Function: FUN_00522f8f @ 0x00522F8F
// Size: 1097 bytes
// ============================================================

// mp_register_network_player (setup phase)
export function FUN_00522f8f(param_1) {
  let iVar1;
  let uVar2;
  let local_314;
  let local_e8;
  let local_1c;
  let local_18;
  let local_14;

  FUN_0059db08(0x4000);
  G.DAT_00655b0a = G.DAT_00655b0b_val | 1;
  for (local_1c = 1; local_1c < 8; local_1c = local_1c + 1) {
    if ((1 << (u8(local_1c) & 0x1f) & (G.DAT_00655b0b_val >>> 0)) === 0) {
      if (G.DAT_00655502[local_1c * 0x30] < 1) {
        G.DAT_00655502[local_1c * 0x30] =
             (~G.DAT_00655502[local_1c * 0x30] + 1) & 0xffff;
      }
      if (G.DAT_00655504[local_1c * 0x30] < 1) {
        G.DAT_00655504[local_1c * 0x30] =
             (~G.DAT_00655504[local_1c * 0x30] + 1) & 0xffff;
      }
      if (G.DAT_00655506[local_1c * 0x30] < 1) {
        G.DAT_00655506[local_1c * 0x30] =
             (~G.DAT_00655506[local_1c * 0x30] + 1) & 0xffff;
      }
      for (local_18 = 0; local_18 < 7; local_18 = local_18 + 1) {
        if (G.DAT_0065550c[local_18 * 4 + local_1c * 0x30] < 1) {
          G.DAT_0065550c[local_18 * 4 + local_1c * 0x30] =
               (~G.DAT_0065550c[local_18 * 4 + local_1c * 0x30] + 1) & 0xffff;
        }
        if (G.DAT_0065550e[local_18 * 4 + local_1c * 0x30] < 1) {
          G.DAT_0065550e[local_18 * 4 + local_1c * 0x30] =
               (~G.DAT_0065550e[local_18 * 4 + local_1c * 0x30] + 1) & 0xffff;
        }
      }
    }
  }
  for (local_18 = 0; local_18 < ((G.DAT_00655b0d + 1) - param_1) | 0; local_18 = local_18 + 1) {
    iVar1 = Math.floor(Math.random() * 0x7fff); // _rand
    local_314 = u8(iVar1 >> 0x1f);
    local_314 =
         (((u8(iVar1) ^ u8(local_314)) - u8(local_314) & 7 ^ u8(local_314)) - u8(local_314));
    if ((1 << (u8(local_314) & 0x1f) & (G.DAT_00655b0a >>> 0)) === 0) {
      G.DAT_00655b0a = G.DAT_00655b0a | u8(1 << (u8(local_314) & 0x1f));
    }
    else {
      local_18 = local_18 + -1;
    }
  }
  if (G.DAT_00631ee4 !== 0) {
    local_14 = 0;
    for (local_314 = 1; local_314 < 8; local_314 = local_314 + 1) {
      if (((1 << (u8(local_314) & 0x1f) & (G.DAT_00655b0b_val >>> 0)) === 0) &&
         ((1 << (u8(local_314) & 0x1f) & (G.DAT_00655b0a >>> 0)) !== 0)) {
        local_14 = local_14 + 1;
        FUN_00421da0(0, local_14);
        FUN_0052263c(9, 0);
        FUN_0040ffa0(s_OPPONENT_006322e8, 1);
        local_e8 = { 4: 0x15 }; // *(local_e8 + 4) = 0x15
        for (local_1c = 0; local_1c < 0x15; local_1c = local_1c + 1) {
          if (G.DAT_006554fe[local_1c * 0x30] === local_314) {
            FUN_0040bbb0();
            FUN_0040ff00(G.DAT_00655504[local_1c * 0x30] | 0);
            FUN_0040fe10();
            FUN_0040fea0();
            FUN_0040ff00(G.DAT_00655502[local_1c * 0x30] | 0);
            FUN_0040fed0();
            FUN_0059edf0(G.DAT_00679640, local_1c, 0);
          }
        }
        iVar1 = -((((G.DAT_006ab19c + -0x1e0 + (G.DAT_006ab19c + -0x1e0 >> 0x1f & 7)) >>> 0) >> 3 | 0) + 1);
        uVar2 = FUN_005226fa(0, iVar1);
        FUN_0059e783(uVar2, iVar1);
        iVar1 = FUN_0040bc80(0);
        FUN_0046e020(0x6a, 0, 0, 0);
        FUN_0055a567();
        if (iVar1 < 0) {
          FUN_005233d8();
          FUN_005233ee();
          return;
        }
        if (iVar1 < 0x15) {
          local_1c = u8(iVar1);
          G.DAT_0062ced0[local_314] = u8(local_1c);
        }
      }
    }
  }
  FUN_005233d8();
  FUN_005233ee();
  return;
}



// ============================================================
// Function: FUN_005233d8 @ 0x005233D8
// Size: 12 bytes
// ============================================================

// CRT destructor thunk
export function FUN_005233d8() {
  FUN_0059df8a();
  return;
}



// ============================================================
// Function: FUN_005233ee @ 0x005233EE
// Size: 14 bytes
// ============================================================

// SEH epilog (no-op in JS)
export function FUN_005233ee() {
  // SEH exception handler restoration — no-op in JS
  return;
}



// ============================================================
// Function: FUN_005233fc @ 0x005233FC
// Size: 2446 bytes
// ============================================================

// mp_register_network_player
export function FUN_005233fc(param_1, param_2) {
  let iVar1;
  let uVar2;
  let bVar3;
  let uVar4;
  let uVar5;
  let local_28;
  let local_24;
  let local_20;
  let local_1c;
  let local_c;
  let local_8;

  if (G.DAT_006ad2f7 === '\0' || G.DAT_006ad2f7 === 0) {
    FUN_005dae6b(7, s_gNetMgr_bServer_00632320, s_D__Ss_Franklinton_startup_multip_006322f4, 0x778);
  }
  uVar4 = G.DAT_006d1da0;
  bVar3 = G.DAT_00655b05;
  uVar2 = G.DAT_00655b03;
  if (((G.DAT_00654fa6 === 0) && (G.DAT_00654fa4 === 0)) && (G.DAT_006ad640 < 2)) {
    iVar1 = param_1[0x18];
    uVar5 = param_1[0x14];
    local_24 = param_1[0x10];
    G.DAT_00655b03 = u8(local_24);
    G.DAT_0064c6a6[s8(u8(local_24)) * 0x594] = uVar5 & 0xffff;
    local_8 = u8(uVar5);
    G.DAT_0064ca92[s8(G.DAT_00655b03) * 0x594] = local_8;
    if (iVar1 !== 0) {
      G.DAT_0064ca92[s8(G.DAT_00655b03) * 0x594] =
           G.DAT_0064ca92[s8(G.DAT_00655b03) * 0x594] + 0x15;
    }
    G.DAT_006d1da0 = s8(G.DAT_00655b03) | 0;
    if (G.DAT_006ad640 < 3) {
      G.DAT_00655b0b_val = G.DAT_00655b0b_val | u8(1 << (u8(local_24) & 0x1f));
    }
    else {
      G.DAT_006c31a8 = G.DAT_006c31a8 | u8(1 << (u8(local_24) & 0x1f));
    }
    local_1c = u8(iVar1);
    G.DAT_006554fc[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30] = local_1c;
    G.DAT_00655502[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30] =
         G.DAT_00655508[u8(G.DAT_006554fc[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30])
         * 2 + G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30];
    uVar5 = FUN_00428b0c(G.DAT_00655502[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30] | 0);
    FUN_005f22d0(G.DAT_0064bcfa + s8(G.DAT_00655b03) * 0xf2, uVar5);
    if (0 < G.DAT_00655502[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30]) {
      G.DAT_00655502[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30] =
           (-G.DAT_00655502[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30]) & 0xffff;
    }
    FUN_005f22d0(G.DAT_0064bcfa + s8(G.DAT_00655b03) * 0xf2, param_1 + 0x20);
    if (param_1[0x1c] !== 0) {
      G.DAT_00655504[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30] =
           (-G.DAT_00655504[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30]) & 0xffff;
      G.DAT_00655506[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30] =
           (-G.DAT_00655506[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30]) & 0xffff;
      for (local_c = 0; local_c < 7; local_c = local_c + 1) {
        if (G.DAT_0065550c[local_c * 4 +
                    G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30 + iVar1 * 2] < 1) {
          local_28 = ~(G.DAT_0065550c[local_c * 4 +
                                   G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30 +
                                   iVar1 * 2] | 0) + 1;
        }
        else {
          local_28 = G.DAT_0065550c[local_c * 4 +
                                  G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30 +
                                  iVar1 * 2] | 0;
        }
        uVar5 = FUN_00428b0c(local_28);
        FUN_005f22d0(G.DAT_0064bd42 + s8(G.DAT_00655b03) * 0xf2 + local_c * 0x18, uVar5);
        G.DAT_0065550c[local_c * 4 +
                  G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30 + iVar1 * 2] =
             (-G.DAT_0065550c[local_c * 4 +
                        G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30 + iVar1 * 2]) & 0xffff;
      }
      FUN_005f22d0(G.DAT_0064bd12 + s8(G.DAT_00655b03) * 0xf2, param_1 + 0x38);
      FUN_005f22d0(G.DAT_0064bd2a + s8(G.DAT_00655b03) * 0xf2, param_1 + 0x50);
      if (param_1[0x68] !== 0) {
        for (local_c = 0; local_c < 7; local_c = local_c + 1) {
          FUN_005f22d0(G.DAT_0064bd42 + s8(G.DAT_00655b03) * 0xf2 + local_c * 0x18,
                       local_c * 0x18 + param_1 + 0x6c);
        }
      }
    }
    G.DAT_0064bcf8[s8(G.DAT_00655b03) * 0xf2] = (param_1[0x114]) & 0xffff;
  }
  else {
    local_24 = param_1[0x10];
    if (G.DAT_006ad640 < 3) {
      G.DAT_00655b0b_val = G.DAT_00655b0b_val | u8(1 << (u8(local_24) & 0x1f));
    }
    else {
      G.DAT_006c31a8 = G.DAT_006c31a8 | u8(1 << (u8(local_24) & 0x1f));
    }
    G.DAT_00655b03 = u8(local_24);
    G.DAT_00655b05 = u8(local_24);
    G.DAT_006d1da0 = local_24;
    FUN_005f22d0(G.DAT_0064bcfa + s8(u8(local_24)) * 0xf2, param_1 + 0x20);
    if (0 < G.DAT_00655502[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30]) {
      G.DAT_00655502[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30] =
           (-G.DAT_00655502[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30]) & 0xffff;
    }
    iVar1 = param_1[0x18];
    G.DAT_0064c6a0[G.DAT_006d1da0 * 0x594] =
         G.DAT_0064c6a0[G.DAT_006d1da0 * 0x594] & 0xfdff;
    if (iVar1 !== 0) {
      G.DAT_0064c6a0[G.DAT_006d1da0 * 0x594] =
           G.DAT_0064c6a0[G.DAT_006d1da0 * 0x594] | 0x200;
    }
    local_1c = u8(iVar1);
    G.DAT_006554fc[G.DAT_0064c6a6[s8(G.DAT_00655b03) * 0x594] * 0x30] = local_1c;
  }
  for (local_20 = 0; (local_20 < 7 && (G.DAT_006ad30c[local_20 * 0x54] !== param_2));
      local_20 = local_20 + 1) {
  }
  if (local_20 < 7) {
    G.DAT_006ad558[s8(G.DAT_00655b03) * 4] = local_20;
    G.DAT_006ad35c[local_20 * 0x15] = s8(G.DAT_00655b03) | 0;
    G.DAT_006ad359[local_20 * 0x54] = 1;
    if (param_2 === 0) {
      G.DAT_006ad358[local_20 * 0x54] = 1;
    }
    else {
      G.DAT_006ad358[local_20 * 0x54] = 0;
    }
    uVar5 = 1;
  }
  else {
    if (G.DAT_006ad640 < 3) {
      G.DAT_00655b0b_val = G.DAT_00655b0b_val & ~u8(1 << (u8(local_24) & 0x1f));
    }
    else {
      G.DAT_006c31a8 = G.DAT_006c31a8 & ~u8(1 << (u8(local_24) & 0x1f));
    }
    FUN_0046b14d(0x31, 0, param_1[0x14], param_1[0x10], 0, 0, 0, 0, 0, 0);
    uVar5 = 0;
  }
  G.DAT_006d1da0 = uVar4;
  G.DAT_00655b05 = bVar3;
  G.DAT_00655b03 = uVar2;
  return uVar5;
}



// ============================================================
// Function: FUN_00523d8a @ 0x00523D8A
// Size: 376 bytes
// ============================================================

// mp_send_network_state_all
export function FUN_00523d8a(param_1) {
  FUN_0046b14d(0x17, param_1, 0, 0, 0, 0, 0, 0, 0, 0);
  FUN_0046b14d(0x18, param_1, 0, 0, 0, 0, 0, 0, 0, 0);
  FUN_0046b14d(0x19, param_1, 0, 0, 0, 0, 0, 0, 0, 0);
  FUN_0046b14d(0x1a, param_1, 0, 0, 0, 0, 0, 0, 0, 0);
  FUN_0046b14d(0x1b, param_1, 0, 0, 0, 0, 0, 0, 0, 0);
  FUN_0046b14d(0x1c, param_1, 0, 0, 0, 0, 0, 0, 0, 0);
  FUN_0046b14d(0x1d, param_1, 0, 0, 0, 0, 0, 0, 0, 0);
  FUN_0046b14d(0x1e, param_1, 0, 0, 0, 0, 0, 0, 0, 0);
  FUN_0046b14d(0x21, param_1, 0, 0, 0, 0, 0, 0, 0, 0);
  FUN_0046b14d(0x22, param_1, 0, 0, 0, 0, 0, 0, 0, 0);
  FUN_0046b14d(0x23, param_1, 0, 0, 0, 0, 0, 0, 0, 0);
  FUN_0046b14d(0x24, param_1, 0, 0, 0, 0, 0, 0, 0, 0);
  return;
}



// ============================================================
// Function: FUN_00523f02 @ 0x00523F02
// Size: 1976 bytes
// ============================================================

// mp_choose_additional_player
export function FUN_00523f02(param_1) {
  let uVar1;
  let iVar2;
  let iVar3;
  let uVar4;
  let uVar5;
  let uVar6;
  let uVar7;
  let local_44c;
  let local_448;
  let local_444;
  let local_440 = new Array(260);
  let local_33c;
  let local_338;
  let local_334;
  let local_330;
  let local_32c = new Array(616);
  let local_c4;
  let local_38;
  let local_34;
  let aiStack_30 = new Array(8);

  FUN_0059db08(0x4000);
  local_330 = 0;
  do {
    FUN_0040bc40(1);
    FUN_0052182c(local_32c, param_1 + 0x25f);
    local_33c = 0;
    for (local_44c = 1; local_44c < 8; local_44c = local_44c + 1) {
      if ((1 << (u8(local_44c) & 0x1f) & (G.DAT_00655b0a >>> 0)) !== 0) {
        FUN_0040bbb0();
        uVar1 = FUN_00493c7d(local_44c);
        FUN_0040bbe0(uVar1);
        FUN_0040fe10();
        FUN_0040fea0();
        uVar1 = FUN_00493b10(local_44c);
        FUN_0040bbe0(uVar1);
        FUN_0040fed0();
        FUN_0040fe10();
        FUN_0040fea0();
        FUN_0040ff30(G.DAT_0064c708[local_44c * 0x594] | 0);
        FUN_0040fe10();
        FUN_0040bc10(0xc5);
        FUN_0040bbe0(G.DAT_00632330);
        FUN_0040ff30(G.DAT_0064c706[local_44c * 0x594] | 0);
        FUN_0040fe10();
        FUN_0040bc10(0xc4);
        FUN_0040bbe0(G.DAT_00632334);
        iVar2 = local_33c;
        local_33c = local_33c + 1;
        local_38 = FUN_0059edf0(G.DAT_00679640, iVar2, 0);
        aiStack_30[local_38[4]] = local_44c;
      }
    }
    local_334 = local_33c;
    local_338 = 0;
    for (local_33c = 0; local_33c < local_334; local_33c = local_33c + 1) {
      local_38 = FUN_0059e7ad(local_33c);
      if (local_38 !== null && local_38 !== 0) {
        local_44c = u8(aiStack_30[local_38[4]]);
        if ((1 << (u8(local_44c) & 0x1f) & (G.DAT_00655b0b_val >>> 0)) === 0) {
          local_338 = local_338 + 1;
          FUN_0059e8db(local_38[4], 0);
          if (local_c4 !== 0) {
            FUN_00421ca0(local_38[4]);
          }
        }
        else {
          FUN_0059e8db(local_38[4], 1);
          if (local_c4 !== 0) {
            FUN_00447210(local_38[4]);
          }
        }
      }
    }
    local_330 = 0;
    for (local_33c = 0; local_33c < local_334; local_33c = local_33c + 1) {
      local_38 = FUN_0059e7ad(local_330);
      if ((local_38 !== null && local_38 !== 0) && ((local_38[0] & 1) === 0)) {
        FUN_0059ea99(local_38[4]);
        if (local_c4 !== 0) {
          FUN_004472f0(local_38[4]);
        }
        break;
      }
      local_330 = local_330 + 1;
      if (local_330 === local_334) {
        local_330 = 0;
      }
    }
    iVar2 = FUN_0040bc80(0);
    if (iVar2 < 0) {
      FUN_005246bf();
      FUN_005246d5();
      return;
    }
    iVar2 = aiStack_30[iVar2];
    local_44c = u8(iVar2);
    G.DAT_00655b0b_val = G.DAT_00655b0b_val | u8(1 << (u8(local_44c) & 0x1f));
    uVar7 = 1;
    uVar6 = 0;
    uVar5 = 0;
    uVar4 = 0;
    G.DAT_006d1da0 = iVar2;
    uVar1 = FUN_00493b10(iVar2, 0, 0, 0, 1);
    iVar3 = FUN_005a632a(G.DAT_006359d4, G.DAT_00632338, 0x17, uVar1, uVar4, uVar5, uVar6, uVar7);
    if (iVar3 === 0) {
      FUN_0052182c(local_32c, param_1 + 0x25f);
      iVar3 = FUN_005a5f34(local_440, 0);
      if (iVar3 === 0) {
        FUN_005f22d0(G.DAT_0064bcfa + iVar2 * 0xf2, local_440);
        if (0 < G.DAT_00655502[G.DAT_0064c6a6[iVar2 * 0x594] * 0x30]) {
          G.DAT_00655502[G.DAT_0064c6a6[iVar2 * 0x594] * 0x30] =
               (-G.DAT_00655502[G.DAT_0064c6a6[iVar2 * 0x594] * 0x30]) & 0xffff;
        }
        iVar2 = FUN_00498a5c(G.DAT_006d1da0);
        if (iVar2 === 0) {
          G.DAT_00655b0b_val = G.DAT_00655b0b_val & ~u8(1 << (u8(local_44c) & 0x1f));
        }
        else {
          FUN_0040ffa0(s_GENDER_00632340, 1);
          FUN_0052182c(local_32c, param_1 + 0x25f);
          FUN_0059ea99(G.DAT_006554fc[G.DAT_0064c6a6[G.DAT_00655b03 * 0x594] * 0x30]);
          FUN_0059e783(0xfffffc19,
                       -((((G.DAT_006ab19c + -0x1e0 + (G.DAT_006ab19c + -0x1e0 >> 0x1f & 7)) >>> 0) >> 3 | 0)
                         + 1));
          iVar2 = FUN_0040bc80(0);
          FUN_0046e020(0x6a, 0, 0, 0);
          if (iVar2 >= 0) {
            local_448 = u8(iVar2);
            G.DAT_006554fc[G.DAT_0064c6a6[G.DAT_00655b03 * 0x594] * 0x30] = local_448;
            G.DAT_0064c6a0[G.DAT_006d1da0 * 0x594] =
                 G.DAT_0064c6a0[G.DAT_006d1da0 * 0x594] & 0xfdff;
            if (iVar2 !== 0) {
              G.DAT_0064c6a0[G.DAT_006d1da0 * 0x594] =
                   G.DAT_0064c6a0[G.DAT_006d1da0 * 0x594] | 0x200;
            }
            for (local_34 = 0; local_34 < 7; local_34 = local_34 + 1) {
              for (local_444 = 0; local_444 < 2; local_444 = local_444 + 1) {
                G.DAT_0065550c[local_34 * 4 +
                          G.DAT_0064c6a6[G.DAT_00655b03 * 0x594] * 0x30 + local_444 * 2] =
                     (G.DAT_00654fe0[local_444 * 4 + local_34 * 8]) & 0xffff;
              }
            }
            FUN_005246bf();
            FUN_005246d5();
            return;
          }
          G.DAT_00655b0b_val = G.DAT_00655b0b_val & ~u8(1 << (u8(local_44c) & 0x1f));
        }
      }
      else {
        G.DAT_00655b0b_val = G.DAT_00655b0b_val & ~u8(1 << (u8(local_44c) & 0x1f));
      }
    }
  } while (true);
}



// ============================================================
// Function: FUN_005246bf @ 0x005246BF
// Size: 12 bytes
// ============================================================

// CRT destructor thunk
export function FUN_005246bf() {
  FUN_0059df8a();
  return;
}



// ============================================================
// Function: FUN_005246d5 @ 0x005246D5
// Size: 15 bytes
// ============================================================

// SEH epilog (no-op in JS)
export function FUN_005246d5() {
  // SEH exception handler restoration — no-op in JS
  return;
}



// ============================================================
// Function: FUN_005261a0 @ 0x005261A0
// Size: 365 bytes
// ============================================================

// civpedia_init_list
export function FUN_005261a0(in_ECX) {
  let local_8;

  if ((in_ECX[0x120] < 0) || (in_ECX[0xab8] <= in_ECX[0x120])) {
    in_ECX[0x120] = 0;
  }
  in_ECX[0x1b34] = in_ECX[0xab8];
  in_ECX[0x1f38] = in_ECX[0x120];
  in_ECX[0x1f3c] = in_ECX[0x120];
  while (G.DAT_006a85a4 % 9 !== 0) {
    in_ECX[0x1f3c] = in_ECX[0x1f3c] + -1;
  }
  for (local_8 = 0; local_8 < in_ECX[0xab8]; local_8 = local_8 + 1) {
    in_ECX[0x1b38 + local_8 * 4] = in_ECX[0xb2c + local_8 * 4];
  }
  FUN_0040fd40(0, (in_ECX[0x1b34] / 9) | 0);
  FUN_0040fcf0((in_ECX[0x1f3c] / 9) | 0);
  if (in_ECX[0x1b34] < 1) {
    in_ECX[0x1f3c] = 0xffffffff;
  }
  FUN_00451bf0();
  FUN_004923c0();
  FUN_004518d0();
  FUN_004f6646();
  return;
}



// ============================================================
// Function: FUN_0052630d @ 0x0052630D
// Size: 1542 bytes
// ============================================================

// civpedia_draw_detail
export function FUN_0052630d(in_ECX) {
  let iVar1;
  let extraout_EAX;
  let uVar2;
  let extraout_EAX_00;
  let sVar3;
  let iVar4;
  let local_88 = new Array(16);
  let local_78 = new Array(16);
  let local_68;
  let local_64 = {};
  let local_50;
  let local_4c;
  let local_48;
  let local_44;
  let local_40;
  let local_3c;
  let local_38;
  let local_34;
  let local_30 = {};
  let local_20;
  let local_1c;
  let local_18 = new Array(16);
  let local_8;

  local_34 = 0;
  FUN_005c00ce(local_18);
  FUN_005c0073(in_ECX[0x5f8]);
  FUN_00451bf0();
  FUN_004f6564(in_ECX[0x5f8], 2);
  local_8 = G.DAT_0067a798;
  local_40 = in_ECX[0x5f8] + 10;
  local_3c = FUN_00407f90(in_ECX[0x5f8]);
  local_1c = FUN_0040ef70();
  local_48 = in_ECX[0x5fc] + 8;
  local_38 = (local_3c / 2 | 0) + -100;
  local_68 = in_ECX[0xb2c + in_ECX[0x120] * 4];
  FUN_005cda06(local_4c, local_44);
  FUN_005cd775(2, 1);
  local_50 = FUN_00451830();
  local_50 = local_50 + G.DAT_0062d858 * 2;
  FUN_005cef31(local_78, G.DAT_006a6668, local_40, local_48);
  FUN_005cd775(local_4c, local_44);
  FUN_005c19ad(G.DAT_00635a1c);
  local_40 = in_ECX[0x5f8] + 10;
  iVar1 = FUN_004bb540();
  local_48 = local_48 + iVar1 * 2 + 5;
  FUN_0040bbb0();
  FUN_0040bc10(0x79);
  FUN_0040fe40();
  FUN_005c0f57(local_8, G.DAT_00679640, local_40, local_48, 5);
  iVar1 = FUN_0040efd0(G.DAT_00679640);
  local_40 = local_40 + iVar1 + 5;
  FUN_0040bbb0();
  if (s8(G.DAT_0064c48e[local_68 * 8]) < 0) {
    FUN_0040bc10(0xe);
    FUN_005c0f57(local_8, G.DAT_00679640, local_40, local_48, 5);
  }
  else {
    FUN_0040ff00(G.DAT_00627684[s8(G.DAT_0064c48e[local_68 * 8]) * 0x10]);
    local_20 = FUN_0040efd0(G.DAT_00679640);
    extraout_EAX = FUN_Timevec_dtor(local_8);
    SetRect(local_30, local_40, local_48, local_40 + local_20 + 5, local_48 + extraout_EAX);
    FUN_00452c14(s8(G.DAT_0064c48e[local_68 * 8]) | 0, local_40, local_48, 0, local_30.left,
                 local_30.top, local_30.right, local_30.bottom);
  }
  local_38 = local_40;
  local_40 = in_ECX[0x5f8] + 10;
  local_48 = local_48 + local_1c;
  FUN_0040bbb0();
  FUN_0040bc10(0x84);
  FUN_0040fe40();
  FUN_005c0f57(local_8, G.DAT_00679640, local_40, local_48, 5);
  FUN_0040bbb0();
  _sprintf(G.DAT_00679640, G.DAT_00632428, u8(G.DAT_0064c48c[local_68 * 8]) * 10);
  FUN_005c0f57(local_8, G.DAT_00679640, local_38, local_48, 5);
  iVar1 = FUN_0040efd0(G.DAT_00679640);
  local_40 = local_38 + iVar1 + 3;
  iVar4 = local_48 + (local_1c / 2 | 0);
  iVar1 = FUN_00451860();
  FUN_005cef31(local_88, G.DAT_006a6668, local_40, iVar4 - (iVar1 / 2 | 0));
  local_40 = in_ECX[0x5f8] + 10;
  local_48 = local_48 + local_1c;
  FUN_0040bbb0();
  FUN_0040bc10(0x9f);
  FUN_0040fe40();
  FUN_005c0f57(local_8, G.DAT_00679640, local_40, local_48, 5);
  FUN_0040bbb0();
  if (local_68 < 0x17) {
    FUN_0040bc10(0xe);
    FUN_005c0f57(local_8, G.DAT_00679640, local_38, local_48, 5);
  }
  else if (s8(G.DAT_0064ba01[local_68]) < 1) {
    FUN_0040bc10(0xe);
    FUN_005c0f57(local_8, G.DAT_00679640, local_38, local_48, 5);
  }
  else {
    uVar2 = FUN_00428b0c(G.DAT_00627684[s8(G.DAT_0064ba01[local_68]) * 0x10]);
    FUN_0040bbe0(uVar2);
    local_20 = FUN_0040efd0(G.DAT_00679640);
    extraout_EAX_00 = FUN_Timevec_dtor(local_8);
    SetRect(local_30, local_38, local_48, local_38 + local_20 + 5, local_48 + extraout_EAX_00);
    FUN_00452c14(s8(G.DAT_0064ba01[local_68]) | 0, local_38, local_48, 0, local_30.left,
                 local_30.top, local_30.right, local_30.bottom);
  }
  local_40 = in_ECX[0x5f8] + 10;
  local_48 = local_48 + local_1c * 2;
  FUN_0059a15d(local_68);
  sVar3 = G.DAT_00679640.length; // _strlen
  if (sVar3 !== 0) {
    iVar1 = in_ECX[0x604];
    iVar4 = FUN_00407f90(in_ECX[0x5f8]);
    SetRect(local_64, local_40, local_48, iVar4 + -10 + local_40, iVar1);
    FUN_005c1167(local_8, G.DAT_00679640, local_64, 5);
  }
  FUN_00452768(in_ECX[0x120]);
  FUN_005c0073(local_18);
  FUN_00408490(in_ECX[0x5f8]);
  return;
}



// ============================================================
// Function: FUN_00526913 @ 0x00526913
// Size: 334 bytes
// ============================================================

// civpedia_select_item
export function FUN_00526913(param_1, in_ECX) {
  let uVar1;
  let local_8;

  if ((G.DAT_006a677c !== 0) && (G.DAT_006ad908 === 0)) {
    for (local_8 = 0;
        (local_8 < in_ECX[0xab8] && (in_ECX[0xb2c + local_8 * 4] !== param_1));
        local_8 = local_8 + 1) {
    }
    if (in_ECX[0xab8] !== local_8) {
      FUN_004f7bd1(3, 1);
      in_ECX[0x120] = local_8;
      in_ECX[0x1f38] = local_8;
      in_ECX[0x124] = 1;
      in_ECX[0x11c] = 1;
      FUN_004f4793();
      FUN_00451bf0();
      uVar1 = FUN_004f8a9b(G.DAT_006a7d4c, local_8);
      FUN_005f22d0(in_ECX[0x618], uVar1);
      FUN_004f6244();
      FUN_0052630d(in_ECX);
      FUN_00408460();
      FUN_004518d0();
      CPropertySheet_EnableStackedTabs(G.DAT_006a66b0, 0x401843);
      FUN_005c61b0();
      CPropertySheet_EnableStackedTabs(G.DAT_006a66b0, 0);
    }
  }
  return;
}



// ============================================================
// Function: FUN_00526ca0 @ 0x00526CA0
// Size: 26152 bytes
// ============================================================

// parley_add_dialog_panel
export function FUN_00526ca0(param_1, param_2, in_ECX) {
  let iVar1;
  let pvVar2;
  let uVar3;
  let extraout_EAX;
  let extraout_EAX_00;
  let extraout_EAX_01;
  let extraout_EAX_02;
  let iVar4;
  let uVar5;
  let local_53c;
  let local_538;
  let local_534;
  let local_530;
  let local_52c;
  let local_528;
  let local_524;
  let local_51c;
  let local_518;
  let local_514;
  let local_50c;
  let local_504;
  let local_4fc;
  let local_4f8;
  let local_4f4;
  let local_4ec;
  let local_4e8;
  let local_4e4;
  let local_4e0;
  let local_4d8;
  let local_4d4;
  let local_4cc;
  let local_4c4;
  let local_4c0;
  let local_4bc;
  let local_4b8;
  let local_4b0;
  let local_4ac;
  let local_4a8;
  let local_4a4;
  let local_498;
  let local_494;
  let local_490;
  let local_48c;
  let local_488;
  let local_484;
  let local_480;
  let local_47c;
  let local_478;
  let local_474;
  let local_470;
  let local_468;
  let local_464;
  let local_45c;
  let local_458;
  let local_44c;
  let local_448;
  let local_444;
  let local_43c;
  let local_434;
  let local_430;
  let local_42c;
  let local_424;
  let local_420;
  let local_418;
  let local_414;
  let local_410;
  let local_40c;
  let local_400;
  let local_3fc;
  let local_3f4;
  let local_3ec;
  let local_3e8;
  let local_3e4;
  let local_3e0;
  let local_3dc;
  let local_3d8;
  let local_3d4;
  let local_3c8;
  let local_3c4;
  let local_3c0;
  let local_3bc;
  let local_3b4;
  let local_3b0;
  let local_3ac;
  let local_3a0;
  let local_39c;
  let local_390;
  let local_38c;
  let local_380;
  let local_37c;
  let local_378;
  let local_370;
  let local_36c;
  let local_368;
  let local_364;
  let local_354;
  let local_34c;
  let local_344;
  let local_33c;
  let local_338;
  let local_330;
  let local_328;
  let local_324;
  let local_31c;
  let local_318;
  let local_314;
  let local_310;
  let local_30c;
  let local_308;
  let local_300;
  let local_2fc;
  let local_2f8;
  let local_2f4;
  let local_2ec;
  let local_2e8;
  let local_2e4;
  let local_2dc;
  let local_2d4;
  let local_2cc;
  let local_2c4;
  let local_2c0;
  let local_2bc;
  let local_2b8;
  let local_2b0;
  let local_2ac;
  let local_2a8;
  let local_2a4;
  let local_2a0;
  let local_29c;
  let local_294;
  let local_290;
  let local_28c;
  let local_288;
  let local_284;
  let local_27c;
  let local_278;
  let local_274;
  let local_270;
  let local_268;
  let local_264;
  let local_260;
  let local_258;
  let local_254;
  let local_250;
  let local_248;
  let local_240;
  let local_23c;
  let local_238;
  let local_230;
  let local_228;
  let local_220;
  let local_21c;
  let local_214;
  let local_210;
  let local_20c;
  let local_208;
  let local_204;
  let local_1fc;
  let local_1f8;
  let local_1f0 = { left: 0, top: 0, right: 0, bottom: 0 };
  let local_1e0;
  let local_1dc;
  let local_1d8;
  let local_1d4;
  let local_1d0;
  let local_1cc;
  let local_1c8;
  let local_1c4;
  let local_1c0;
  let local_1bc;
  let local_1b8;
  let local_1b4;
  let local_1b0;
  let local_1ac = new Array(4);
  let local_19c;
  let local_198 = new Array(4);
  let local_188;
  let local_184;
  let local_180;
  let local_17c = { left: 0, top: 0, right: 0, bottom: 0 };
  let local_16c;
  let local_168;
  let local_164;
  let local_160;
  let local_15c;
  let local_158 = new Array(32);
  let local_138;
  let local_134;
  let local_130;
  let local_12c;
  let local_128;
  let local_124;
  let local_120;
  let local_11c;
  let local_118;
  let local_114;
  let local_110;
  let local_10c;
  let local_108;
  let local_104;
  let local_100;
  let local_fc;
  let local_f8;
  let local_f4;
  let local_f0;
  let local_ec;
  let local_e8;
  let local_e4;
  let local_e0;
  let local_dc;
  let local_d8;
  let local_d4;
  let local_d0;
  let local_cc;
  let local_c8;
  let local_c4;
  let local_c0;
  let local_bc;
  let local_b8;
  let local_b4 = new Array(4);
  let local_a4;
  let local_a0;
  let local_9c = new Array(80);
  let local_4c;
  let local_48;
  let local_44;
  let local_40;
  let local_3c;
  let local_38;
  let local_34;
  let local_30 = 0;
  let local_2c;
  let local_28 = 0;
  let local_24 = { left: 0, top: 0, right: 0, bottom: 0 };
  let local_14;

  if ((param_2 !== 0) && (param_2 !== 1)) {
    FUN_005dae6b(7, s_leftright_00632458,
                 s_D__Ss_Franklinton_parleywin_0063242c, 0xc);
  }
  if (param_1 < 0) {
    FUN_005dae6b(7, s_theDialog_006324ac,
                 s_D__Ss_Franklinton_parleywin_00632480, 0xd);
  }
  local_28 = 0;
  local_30 = 0;
  in_ECX[0x1e4 + param_2 * 4] = 0;
  if (param_2 === 0) {
    local_2c = in_ECX[0x140] + G.DAT_0062d858;
  }
  else if (param_2 === 1) {
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_2c = in_ECX[0x140] + (iVar1 / 2 | 0) + G.DAT_0062d858;
  }
  if (param_1 === 0xe) {
    local_28 = 1;
    switch (in_ECX[0x20c + param_2 * 4]) {
    case 0: param_1 = 9; break;
    case 1: param_1 = 8; break;
    case 2: param_1 = 10; break;
    case 3: param_1 = 0x11; break;
    case 4: param_1 = 0xd; break;
    case 5: param_1 = 0xc; break;
    case 6: param_1 = 5; break;
    }
  }
  else if (param_1 === 0x10) {
    local_30 = 1;
    switch (in_ECX[0x214]) {
    case 0: param_1 = 9; break;
    case 1: param_1 = 8; break;
    case 2: param_1 = 10; break;
    case 3: param_1 = 0x11; break;
    case 4: param_1 = 0xd; break;
    case 5: param_1 = 0xc; break;
    case 6: param_1 = 5; break;
    }
  }
  FUN_004b888e(param_2);
  in_ECX[0x200 + param_2 * 4] = param_1;
  if (((G.DAT_0067a8bc === 0) && (param_1 !== 1)) && (param_1 !== 0)) {
    if (param_1 === 0x14) {
      FUN_0043c5f0();
      FUN_00453c40();
      FUN_0043c5f0();
      FUN_00453c40();
      FUN_0043c5f0();
      FUN_0040f380();
      FUN_0043c5f0();
      FUN_0040f380();
      FUN_0040f380();
    }
    else {
      FUN_0040f380();
      FUN_00453c80();
      FUN_0043c5f0();
      FUN_00453c40();
      FUN_0040f380();
      FUN_0043c5f0();
      FUN_0040f380();
      FUN_0043c5f0();
      FUN_0043c5f0();
    }
  }
  switch (param_1) {
  case 0:
    break;
  case 1:
    if (((G.DAT_00655c16 === -1) && (iVar1 = FUN_00453e51(G.DAT_006d1da0, 9), iVar1 === 0)) &&
       (G.DAT_00655b07 === 0)) {
      local_c8 = 0;
    }
    else {
      local_c8 = 1;
    }
    local_cc = local_2c;
    local_d8 = in_ECX[0x140] + G.DAT_0062d858;
    local_3c = in_ECX[0x144] + G.DAT_0062d85c;
    local_38 = local_d8;
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_34 = ((iVar1 + G.DAT_0062d858 * -2) / 3) | 0;
    if (in_ECX[0x154] === 0) {
      local_370 = FUN_0040ef70();
    }
    else {
      local_370 = FUN_0040ef70();
    }
    local_14 = G.DAT_0062d85c * 2 + local_370;
    if (in_ECX[0x154] === 0) {
      local_220 = G.DAT_0069b030;
    }
    else {
      local_220 = G.DAT_0069b028;
    }
    FUN_005d25a8(local_220);
    pvVar2 = operator_new(0x3c); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_228 = 0;
    }
    else {
      local_228 = FUN_0040f8b0();
    }
    in_ECX[0x2e8] = local_228;
    if (in_ECX === 0) {
      local_378 = 0;
    }
    else {
      local_378 = in_ECX + 0x48;
    }
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xba4]);
    FUN_00530ee0(local_378, 0x415, local_38, local_3c, local_34, uVar3);
    FUN_0040fad0(G.DAT_0068aedc);
    FUN_00530fb0();
    FUN_0040faa0(LAB_00403d28);
    pvVar2 = operator_new(0x3c); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_230 = 0;
    }
    else {
      local_230 = FUN_0040f8b0();
    }
    in_ECX[0x2ec] = local_230;
    if (in_ECX === 0) {
      local_37c = 0;
    }
    else {
      local_37c = in_ECX + 0x48;
    }
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xd84]);
    FUN_00530ee0(local_37c, 0x430, local_34 + local_38, local_3c, local_34, uVar3);
    FUN_0040fad0(G.DAT_0068aee0);
    FUN_00530fb0();
    FUN_0040faa0(LAB_00403d28);
    pvVar2 = operator_new(0x3c); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_238 = 0;
    }
    else {
      local_238 = FUN_0040f8b0();
    }
    in_ECX[0x2f0] = local_238;
    if (in_ECX === 0) {
      local_380 = 0;
    }
    else {
      local_380 = in_ECX + 0x48;
    }
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xba8]);
    FUN_00530ee0(local_380, 0x416, local_34 * 2 + local_38, local_3c, local_34, uVar3);
    FUN_0040fad0(G.DAT_0068aee4);
    FUN_00530fb0();
    FUN_0040faa0(LAB_00403d28);
    local_3c = local_3c + local_14;
    FUN_005c19ad(10);
    if (in_ECX[0x154] === 0) {
      local_23c = G.DAT_0069b030;
    }
    else {
      local_23c = G.DAT_0069b028;
    }
    uVar5 = 5;
    iVar1 = local_d8;
    iVar4 = local_3c;
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xd2c], local_d8, local_3c, 5);
    FUN_005c0f57(local_23c, uVar3, iVar1, iVar4, uVar5);
    if (in_ECX[0x154] === 0) {
      local_240 = G.DAT_0069b030;
    }
    else {
      local_240 = G.DAT_0069b028;
    }
    uVar5 = 5;
    iVar4 = local_cc - ((in_ECX[0x154] === 0 ? 0 : -1) & 0x17);
    iVar1 = local_3c;
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xd2c], iVar4, local_3c, 5);
    FUN_005c0f57(local_240, uVar3, iVar4, iVar1, uVar5);
    local_38 = local_d8;
    if (in_ECX[0x154] === 0) {
      local_c4 = 5;
    }
    else {
      local_c4 = 0x17;
    }
    local_3c = local_3c + local_14;
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_34 = ((iVar1 + G.DAT_0062d858 * -2) / 2) | 0;
    local_d4 = 0;
    for (local_d0 = 1; local_d0 < 8; local_d0 = local_d0 + 1) {
      if (((G.DAT_006d1da0 === local_d0) || ((1 << (u8(local_d0) & 0x1f) & (G.DAT_00655b0a >>> 0)) === 0))
         || (((1 << (u8(local_d0) & 0x1f) & (G.DAT_00655b0b_val >>> 0)) === 0 ||
             (((local_c8 === 0 && (G.DAT_00654c7a === 0)) &&
              ((G.DAT_0064c6c0[G.DAT_006d1da0 * 4 + local_d0 * 0x594] & 1) === 0)))))) {
        G.DAT_0068aee8[local_d0 * 4] = 0;
        G.DAT_0068af08[local_d0 * 4] = 0;
      }
      else {
        pvVar2 = operator_new(0x3c); // DEVIATION: Win32 operator new
        if (pvVar2 === 0 || pvVar2 === null) {
          local_248 = 0;
        }
        else {
          local_248 = FUN_0040f8b0();
        }
        in_ECX[0x2c8 + local_d0 * 4] = local_248;
        if (in_ECX === 0) {
          local_38c = 0;
        }
        else {
          local_38c = in_ECX + 0x48;
        }
        FUN_00530ee0(local_38c, local_d0 + 0x428, local_38 + local_c4, local_3c, 0x23,
                     G.DAT_006324dc);
        FUN_0040fad0(G.DAT_0068af08[local_d0 * 4]);
        FUN_00530fb0();
        FUN_0040faa0(LAB_00403657);
        pvVar2 = operator_new(0x3c); // DEVIATION: Win32 operator new
        if (pvVar2 === 0 || pvVar2 === null) {
          local_250 = 0;
        }
        else {
          local_250 = FUN_0040f8b0();
        }
        in_ECX[0x2a8 + local_d0 * 4] = local_250;
        if (in_ECX === 0) {
          local_390 = 0;
        }
        else {
          local_390 = in_ECX + 0x48;
        }
        uVar3 = FUN_00493d13(local_d0);
        FUN_00530ee0(local_390, local_d0 + 0x416, local_38 + local_c4 + 0x23, local_3c,
                     (local_34 + -0x23) - local_c4, uVar3);
        FUN_0040fad0(G.DAT_0068aee8[local_d0 * 4]);
        FUN_00530fb0();
        FUN_0040faa0(LAB_00403d28);
        local_d4 = local_d4 + 1;
        if ((local_d4 & 1) === 0) {
          local_38 = local_d8;
        }
        else {
          local_38 = local_cc;
        }
        if ((local_d4 & 1) === 0) {
          if (in_ECX[0x154] === 0) {
            local_c4 = 5;
          }
          else {
            local_c4 = 0x17;
          }
        }
        else if (in_ECX[0x154] === 0) {
          local_c4 = 5;
        }
        else {
          local_c4 = 0;
        }
        if ((local_d4 & 1) === 0) {
          local_3c = local_3c + local_14;
        }
      }
    }
    break;
  case 2:
    local_b4[0] = FUN_00428b0c(G.DAT_00628420[0xb90]);
    local_b4[1] = FUN_00428b0c(G.DAT_00628420[0xb94]);
    local_b4[2] = FUN_00428b0c(G.DAT_00628420[0xb98]);
    local_b4[3] = FUN_00428b0c(G.DAT_00628420[0xb9c]);
    local_a4 = FUN_00428b0c(G.DAT_00628420[0xba0]);
    FUN_005c19ad(10);
    if (in_ECX[0x154] === 0) {
      uVar3 = FUN_00428b0c(G.DAT_00628420[0xb8c]);
      local_c0 = FUN_0040efd0(uVar3);
    }
    else {
      uVar3 = FUN_00428b0c(G.DAT_00628420[0xb8c]);
      local_c0 = FUN_0040efd0(uVar3);
    }
    iVar4 = (in_ECX[0x140] + ((in_ECX[0x148] - in_ECX[0x140]) / 2 | 0)) -
            (local_c0 / 2 | 0);
    iVar1 = in_ECX[0x144] + G.DAT_0062d85c;
    if (in_ECX[0x154] === 0) {
      local_210 = G.DAT_0069b020;
    }
    else {
      local_210 = G.DAT_0069b018;
    }
    uVar5 = 5;
    local_3c = iVar1;
    local_38 = iVar4;
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xb8c], iVar4, iVar1, 5);
    FUN_005c0f57(local_210, uVar3, iVar4, iVar1, uVar5);
    if (in_ECX[0x154] === 0) {
      local_c0 = FUN_0040efd0(local_b4[0]);
    }
    else {
      local_c0 = FUN_0040efd0(local_b4[0]);
    }
    for (local_b8 = 1; local_b8 < 5; local_b8 = local_b8 + 1) {
      if (in_ECX[0x154] === 0) {
        local_bc = FUN_0040efd0(local_b4[local_b8]);
      }
      else {
        local_bc = FUN_0040efd0(local_b4[local_b8]);
      }
      if (local_c0 < local_bc) {
        local_c0 = local_bc;
      }
    }
    local_2c = (in_ECX[0x140] + ((in_ECX[0x148] - in_ECX[0x140]) / 2 | 0)) -
               (local_c0 / 2 | 0);
    in_ECX[0x3ac] = local_2c;
    if (local_38 < local_2c) {
      local_2c = local_38;
      in_ECX[0x3ac] = local_38;
    }
    if (in_ECX[0x154] === 0) {
      local_214 = G.DAT_0069b020;
    }
    else {
      local_214 = G.DAT_0069b018;
    }
    FUN_005d4167(local_214);
    pvVar2 = operator_new(0x50); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_21c = 0;
    }
    else {
      local_21c = FUN_00531010();
    }
    in_ECX[0x36c + param_2 * 4] = local_21c;
    local_38 = local_2c;
    local_34 = local_c0 + (local_c0 / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_364 = FUN_0040ef70();
    }
    else {
      local_364 = FUN_0040ef70();
    }
    local_3c = in_ECX[0x144] + G.DAT_0062d85c * 2 + local_364;
    local_14 = in_ECX[0x1e0] - local_3c;
    SetRect(local_24, local_38, local_3c, local_34 + local_38, local_14 + local_3c); // DEVIATION: Win32 SetRect
    if (in_ECX === 0) {
      local_368 = 0;
    }
    else {
      local_368 = in_ECX + 0x48;
    }
    FUN_005310a0(local_368, 0x3f3, local_24, 1, 5, local_b4);
    if (in_ECX[0x208] === -1) {
      local_36c = 0;
    }
    else {
      local_36c = in_ECX[0x208];
    }
    FUN_004472f0(local_36c);
    FUN_005311e0(LAB_00401d75);
    in_ECX[0x1e4 + param_2 * 4] = 1;
    break;
  case 3:
    local_f4 = FUN_00428b0c(G.DAT_00628420[0xbac]);
    local_f0 = FUN_00428b0c(G.DAT_00628420[0xbb0]);
    local_ec = FUN_00428b0c(G.DAT_00628420[0xbb4]);
    local_e8 = FUN_00428b0c(G.DAT_00628420[3000]);
    local_e4 = FUN_00428b0c(G.DAT_00628420[0xbbc]);
    local_e0 = FUN_00428b0c(G.DAT_00628420[0xbc0]);
    local_dc = FUN_00428b0c(G.DAT_00628420[0xd08]);
    FUN_005c19ad(10);
    local_38 = local_2c;
    iVar1 = in_ECX[0x144] + G.DAT_0062d85c;
    if (in_ECX[0x154] === 0) {
      local_254 = G.DAT_0069b030;
    }
    else {
      local_254 = G.DAT_0069b028;
    }
    uVar5 = 5;
    iVar4 = local_2c;
    local_3c = iVar1;
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xc0c], local_2c, iVar1, 5);
    FUN_005c0f57(local_254, uVar3, iVar4, iVar1, uVar5);
    if (in_ECX[0x154] === 0) {
      local_258 = G.DAT_0069b030;
    }
    else {
      local_258 = G.DAT_0069b028;
    }
    FUN_005d4167(local_258);
    pvVar2 = operator_new(0x50); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_260 = 0;
    }
    else {
      local_260 = FUN_00531010();
    }
    in_ECX[0x36c + param_2 * 4] = local_260;
    local_38 = local_2c;
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_34 = ((iVar1 + G.DAT_0062d858 * -2) / 2) | 0;
    if (in_ECX[0x154] === 0) {
      local_39c = FUN_0040ef70();
    }
    else {
      local_39c = FUN_0040ef70();
    }
    local_3c = in_ECX[0x144] + G.DAT_0062d85c + local_39c;
    SetRect(local_24, local_38, local_3c, local_34 + local_38, in_ECX[0x1e0]); // DEVIATION: Win32 SetRect
    if (in_ECX === 0) {
      local_3a0 = 0;
    }
    else {
      local_3a0 = in_ECX + 0x48;
    }
    FUN_005310a0(local_3a0, 0x3f4, local_24, 1, 7, [local_f4, local_f0, local_ec, local_e8, local_e4, local_e0, local_dc]);
    if (in_ECX[0x210] === 0) {
      FUN_00447210(0);
    }
    else if (in_ECX[0x210] === 6) {
      FUN_00447210(6);
    }
    FUN_004472f0(in_ECX[0x20c]);
    FUN_005311e0(LAB_00401d75);
    FUN_005311b0(LAB_004035d0);
    in_ECX[0x1e4 + param_2 * 4] = 1;
    break;
  case 4:
    local_110 = FUN_00428b0c(G.DAT_00628420[0xbac]);
    local_10c = FUN_00428b0c(G.DAT_00628420[0xbb0]);
    local_108 = FUN_00428b0c(G.DAT_00628420[0xbb4]);
    local_104 = FUN_00428b0c(G.DAT_00628420[3000]);
    local_100 = FUN_00428b0c(G.DAT_00628420[0xbbc]);
    local_fc = FUN_00428b0c(G.DAT_00628420[0xbc4]);
    local_f8 = FUN_00428b0c(G.DAT_00628420[0xd08]);
    FUN_005c19ad(10);
    local_38 = local_2c;
    iVar1 = in_ECX[0x144] + G.DAT_0062d85c;
    if (in_ECX[0x154] === 0) {
      local_264 = G.DAT_0069b030;
    }
    else {
      local_264 = G.DAT_0069b028;
    }
    uVar5 = 5;
    iVar4 = local_2c;
    local_3c = iVar1;
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xc10], local_2c, iVar1, 5);
    FUN_005c0f57(local_264, uVar3, iVar4, iVar1, uVar5);
    if (in_ECX[0x154] === 0) {
      local_268 = G.DAT_0069b030;
    }
    else {
      local_268 = G.DAT_0069b028;
    }
    FUN_005d4167(local_268);
    pvVar2 = operator_new(0x50); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_270 = 0;
    }
    else {
      local_270 = FUN_00531010();
    }
    in_ECX[0x36c + param_2 * 4] = local_270;
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_34 = ((iVar1 + G.DAT_0062d858 * -2) / 2) | 0;
    local_38 = local_2c;
    if (in_ECX[0x154] === 0) {
      local_3ac = FUN_0040ef70();
    }
    else {
      local_3ac = FUN_0040ef70();
    }
    local_3c = in_ECX[0x144] + G.DAT_0062d85c + local_3ac;
    SetRect(local_24, local_38, local_3c, local_34 + local_38, in_ECX[0x1e0]); // DEVIATION: Win32 SetRect
    if (in_ECX === 0) {
      local_3b0 = 0;
    }
    else {
      local_3b0 = in_ECX + 0x48;
    }
    FUN_005310a0(local_3b0, 0x3f5, local_24, 1, 7, [local_110, local_10c, local_108, local_104, local_100, local_fc, local_f8]);
    if (in_ECX[0x20c] === 0) {
      FUN_00447210(0);
    }
    else if (in_ECX[0x20c] === 6) {
      FUN_00447210(6);
    }
    FUN_004472f0(in_ECX[0x210]);
    FUN_005311e0(LAB_00401d75);
    FUN_005311b0(LAB_004035d0);
    in_ECX[0x1e4 + param_2 * 4] = 1;
    break;
  case 5:
    local_120 = FUN_00428b0c(G.DAT_00628420[0xbcc]);
    local_11c = FUN_00428b0c(G.DAT_00628420[0xbd0]);
    local_118 = FUN_00428b0c(G.DAT_00628420[0xbd4]);
    local_114 = FUN_00428b0c(G.DAT_00628420[0xbd8]);
    FUN_005c19ad(10);
    local_38 = local_2c;
    local_3c = in_ECX[0x144] + G.DAT_0062d85c;
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_34 = ((iVar1 + G.DAT_0062d858 * -2) / 2 | 0) + G.DAT_0062d858 * -5;
    if ((local_28 !== 0) || (local_30 !== 0)) {
      FUN_004aef20(G.DAT_00679640);
      if (param_2 === 0) {
        local_3b4 = FUN_00428b0c(G.DAT_00628420[0xc0c]);
      }
      else {
        local_3b4 = FUN_00428b0c(G.DAT_00628420[0xc10]);
      }
      FUN_00414d70(local_3b4);
      if (in_ECX[0x154] === 0) {
        local_274 = G.DAT_0069b030;
      }
      else {
        local_274 = G.DAT_0069b028;
      }
      FUN_005c0f57(local_274, G.DAT_00679640, local_38, local_3c, 5);
      if (in_ECX[0x154] === 0) {
        local_3bc = FUN_0040ef70();
      }
      else {
        local_3bc = FUN_0040ef70();
      }
      local_3c = local_3c + G.DAT_0062d85c + local_3bc;
    }
    if ((local_28 === 0) && (local_30 === 0)) {
      if (in_ECX[0x154] === 0) {
        local_3c4 = FUN_0040ef70();
      }
      else {
        local_3c4 = FUN_0040ef70();
      }
      local_14 = local_3c4 * 5 + G.DAT_0062d85c * 4;
    }
    else {
      if (in_ECX[0x154] === 0) {
        local_3c0 = FUN_0040ef70();
      }
      else {
        local_3c0 = FUN_0040ef70();
      }
      local_14 = G.DAT_0062d85c * 3 + local_3c0 * 4;
    }
    local_3c = in_ECX[0x144] +
               (((in_ECX[0x1e0] - in_ECX[0x144]) - local_14) / 2 | 0);
    if (local_30 === 0 && local_28 === 0) {
      local_3c8 = FUN_00428b0c(G.DAT_00628420[0xb90]);
    }
    else {
      local_3c8 = FUN_00428b0c(G.DAT_00628420[0xd08]);
    }
    if (in_ECX[0x154] === 0) {
      local_278 = G.DAT_0069b030;
    }
    else {
      local_278 = G.DAT_0069b028;
    }
    FUN_005c0f57(local_278, local_3c8, local_38, local_3c, 5);
    if (in_ECX[0x154] === 0) {
      local_27c = G.DAT_0069b030;
    }
    else {
      local_27c = G.DAT_0069b028;
    }
    FUN_005d4167(local_27c);
    pvVar2 = operator_new(0x50); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_284 = 0;
    }
    else {
      local_284 = FUN_00531010();
    }
    in_ECX[0x36c + param_2 * 4] = local_284;
    local_38 = local_2c;
    if (in_ECX[0x154] === 0) {
      local_3d4 = FUN_0040ef70();
    }
    else {
      local_3d4 = FUN_0040ef70();
    }
    local_3c = local_3c + G.DAT_0062d85c + local_3d4;
    if ((local_28 === 0) && (local_30 === 0)) {
      if (in_ECX[0x154] === 0) {
        local_3dc = FUN_0040ef70();
      }
      else {
        local_3dc = FUN_0040ef70();
      }
      local_14 = G.DAT_0062d85c * 3 + local_3dc * 4;
    }
    else {
      if (in_ECX[0x154] === 0) {
        local_3d8 = FUN_0040ef70();
      }
      else {
        local_3d8 = FUN_0040ef70();
      }
      local_14 = local_3d8 * 3 + G.DAT_0062d85c * 2;
    }
    SetRect(local_24, local_38, local_3c, local_34 + local_38, local_14 + local_3c); // DEVIATION: Win32 SetRect
    if (in_ECX === 0) {
      local_3e0 = 0;
    }
    else {
      local_3e0 = in_ECX + 0x48;
    }
    FUN_005310a0(local_3e0, 0x3f6, local_24, 1, (local_30 === 0 && local_28 === 0 ? 1 : 0) + 3,
                 [local_120, local_11c, local_118, local_114]);
    if (((G.DAT_0064c6c0[in_ECX[0x118] * 4 + G.DAT_006d1da0 * 0x594] & 2) !== 0) &&
       (FUN_00447210(0), in_ECX[0x218] === 0)) {
      in_ECX[0x218] = in_ECX[0x218] + 1;
    }
    if (((G.DAT_0064c6c0[in_ECX[0x118] * 4 + G.DAT_006d1da0 * 0x594] & 4) !== 0) &&
       (FUN_00447210(1), in_ECX[0x218] === 1)) {
      in_ECX[0x218] = in_ECX[0x218] + 1;
    }
    if (((G.DAT_0064c6c0[in_ECX[0x118] * 4 + G.DAT_006d1da0 * 0x594] & 8) !== 0) &&
       (FUN_00447210(2), in_ECX[0x218] === 2)) {
      if (local_30 === 0 && local_28 === 0) {
        in_ECX[0x218] = 3;
      }
      else {
        in_ECX[0x218] = 0;
      }
    }
    if ((((local_28 === 0) && (local_30 === 0)) &&
        ((G.DAT_0064c6c0[in_ECX[0x118] * 4 + G.DAT_006d1da0 * 0x594] & 0xe) === 0)) &&
       (FUN_00447210(3), in_ECX[0x218] === 3)) {
      in_ECX[0x218] = 0;
    }
    FUN_004472f0(in_ECX[0x218]);
    FUN_005311e0(LAB_00401d75);
    FUN_005311b0(LAB_00403c2e);
    in_ECX[0x1e4 + param_2 * 4] = 1;
    break;
  case 6:
    FUN_005c19ad(10);
    local_38 = local_2c;
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_34 = ((iVar1 + G.DAT_0062d858 * -2) / 2 | 0) - G.DAT_0062d858;
    if (in_ECX[0x154] === 0) {
      local_3e4 = FUN_0040ef70();
    }
    else {
      local_3e4 = FUN_0040ef70();
    }
    if (in_ECX[0x154] === 0) {
      local_3e8 = FUN_0040ef70();
    }
    else {
      local_3e8 = FUN_0040ef70();
    }
    local_14 = G.DAT_0062d85c * 2 + local_3e4 * 4 + local_3e8;
    local_3c = in_ECX[0x144] +
               (((in_ECX[0x1e0] - in_ECX[0x144]) - local_14) / 2 | 0);
    SetRect(local_24, local_38, local_3c, local_34 + local_38, local_14 + local_3c); // DEVIATION: Win32 SetRect
    FUN_005a9780(G.DAT_0067a7a8);
    FUN_00408700(local_24, 10);
    FUN_004bb800(local_24, 1, 1);
    FUN_00408700(local_24, 10);
    iVar1 = FUN_00407f90(local_24);
    local_38 = local_24.left + (iVar1 / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_3ec = FUN_0040ef70();
    }
    else {
      local_3ec = FUN_0040ef70();
    }
    iVar1 = local_3c + G.DAT_0062d85c + (local_3ec / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_288 = G.DAT_0069b030;
    }
    else {
      local_288 = G.DAT_0069b028;
    }
    uVar5 = 0;
    iVar4 = local_38;
    local_3c = iVar1;
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xbdc], local_38, iVar1, 0);
    FUN_005c0f57(local_288, uVar3, iVar4, iVar1, uVar5);
    local_24.bottom = local_24.bottom + G.DAT_0062d85c * -2;
    local_24.left = local_24.left + G.DAT_0062d858 * 2;
    local_24.right = local_24.right + G.DAT_0062d858 * -2;
    if (in_ECX[0x154] === 0) {
      local_3f4 = FUN_0040ef70();
    }
    else {
      local_3f4 = FUN_0040ef70();
    }
    local_24.top = local_24.top + G.DAT_0062d85c + local_3f4;
    FUN_00408700(local_24, 10);
    FUN_004bb800(local_24, 1, 1);
    FUN_005c0333(local_24, G.DAT_00635a18);
    iVar1 = FUN_00407f90(local_24);
    local_38 = local_24.left + (iVar1 / 2 | 0);
    iVar1 = FUN_00407fc0(local_24);
    local_3c = local_24.top + (iVar1 / 2 | 0);
    FUN_0040bbb0();
    if ((G.DAT_0064c6c0[in_ECX[0x118] * 4 + G.DAT_006d1da0 * 0x594] & 8) === 0) {
      if ((G.DAT_0064c6c0[in_ECX[0x118] * 4 + G.DAT_006d1da0 * 0x594] & 4) === 0) {
        if ((G.DAT_0064c6c0[in_ECX[0x118] * 4 + G.DAT_006d1da0 * 0x594] & 2) === 0) {
          if ((G.DAT_0064c6c1[in_ECX[0x118] * 4 + G.DAT_006d1da0 * 0x594] & 0x20) === 0) {
            uVar3 = FUN_00428b0c(G.DAT_00628420[0xbec]);
            FUN_0040bbe0(uVar3);
          }
          else {
            uVar3 = FUN_00428b0c(G.DAT_00628420[0xbe8]);
            FUN_0040bbe0(uVar3);
          }
        }
        else {
          uVar3 = FUN_00428b0c(G.DAT_00628420[0xbcc]);
          FUN_0040bbe0(uVar3);
        }
      }
      else {
        uVar3 = FUN_00428b0c(G.DAT_00628420[0xbe4]);
        FUN_0040bbe0(uVar3);
      }
    }
    else {
      uVar3 = FUN_00428b0c(G.DAT_00628420[0xbe0]);
      FUN_0040bbe0(uVar3);
    }
    if (in_ECX[0x154] === 0) {
      local_28c = G.DAT_0069b030;
    }
    else {
      local_28c = G.DAT_0069b028;
    }
    FUN_005c0f57(local_28c, G.DAT_00679640, local_38, local_3c, 0);
    in_ECX[0x1e4 + param_2 * 4] = 1;
    break;
  case 7:
    local_134 = FUN_00428b0c(G.DAT_00628420[0xbf4]);
    local_130 = FUN_00428b0c(G.DAT_00628420[0xbf8]);
    local_12c = FUN_00428b0c(G.DAT_00628420[0xbfc]);
    local_128 = FUN_00428b0c(G.DAT_00628420[0xc00]);
    local_124 = FUN_00428b0c(G.DAT_00628420[0xc04]);
    FUN_005c19ad(10);
    local_38 = local_2c;
    if (in_ECX[0x154] === 0) {
      local_3fc = FUN_0040ef70();
    }
    else {
      local_3fc = FUN_0040ef70();
    }
    if (in_ECX[0x154] === 0) {
      local_400 = FUN_0040ef70();
    }
    else {
      local_400 = FUN_0040ef70();
    }
    local_14 = G.DAT_0062d85c * 5 + local_3fc * 5 + local_400;
    iVar1 = in_ECX[0x144] +
            (((in_ECX[0x1e0] - in_ECX[0x144]) - local_14) / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_290 = G.DAT_0069b030;
    }
    else {
      local_290 = G.DAT_0069b028;
    }
    uVar5 = 5;
    iVar4 = local_38;
    local_3c = iVar1;
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xbf0], local_38, iVar1, 5);
    FUN_005c0f57(local_290, uVar3, iVar4, iVar1, uVar5);
    if (in_ECX[0x154] === 0) {
      local_294 = G.DAT_0069b030;
    }
    else {
      local_294 = G.DAT_0069b028;
    }
    FUN_005d4167(local_294);
    pvVar2 = operator_new(0x50); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_29c = 0;
    }
    else {
      local_29c = FUN_00531010();
    }
    in_ECX[0x36c + param_2 * 4] = local_29c;
    local_38 = local_2c;
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_34 = ((iVar1 + G.DAT_0062d858 * -2) / 2) | 0;
    if (in_ECX[0x154] === 0) {
      local_40c = FUN_0040ef70();
    }
    else {
      local_40c = FUN_0040ef70();
    }
    local_3c = local_3c + G.DAT_0062d85c + local_40c;
    if (in_ECX[0x154] === 0) {
      local_410 = FUN_0040ef70();
    }
    else {
      local_410 = FUN_0040ef70();
    }
    local_14 = local_410 * 5 + G.DAT_0062d85c * 4;
    SetRect(local_24, local_38, local_3c, local_34 + local_38, local_14 + local_3c); // DEVIATION: Win32 SetRect
    if (in_ECX === 0) {
      local_414 = 0;
    }
    else {
      local_414 = in_ECX + 0x48;
    }
    FUN_005310a0(local_414, 0x3f7, local_24, 1, 5, [local_134, local_130, local_12c, local_128, local_124]);
    FUN_004472f0(in_ECX[0x220]);
    FUN_005311e0(LAB_00401d75);
    FUN_005311b0(LAB_004010ff);
    in_ECX[0x1e4 + param_2 * 4] = 1;
    break;
  case 8:
    if (local_28 === 0) {
      in_ECX[0x10420 + param_2 * 4] = 7;
    }
    else {
      in_ECX[0x10420 + param_2 * 4] = 6;
    }
    FUN_005c19ad(10);
    local_38 = local_2c;
    local_3c = in_ECX[0x144] + G.DAT_0062d85c;
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_34 = ((iVar1 + G.DAT_0062d858 * -2) / 2 | 0) + G.DAT_0062d858 * -5;
    if (local_28 !== 0) {
      FUN_004aef20(G.DAT_00679640);
      if (param_2 === 0) {
        local_418 = FUN_00428b0c(G.DAT_00628420[0xc0c]);
      }
      else {
        local_418 = FUN_00428b0c(G.DAT_00628420[0xc10]);
      }
      FUN_00414d70(local_418);
      if (in_ECX[0x154] === 0) {
        local_2a0 = G.DAT_0069b030;
      }
      else {
        local_2a0 = G.DAT_0069b028;
      }
      FUN_005c0f57(local_2a0, G.DAT_00679640, local_38, local_3c, 5);
      if (in_ECX[0x154] === 0) {
        local_420 = FUN_0040ef70();
      }
      else {
        local_420 = FUN_0040ef70();
      }
      local_3c = local_3c + G.DAT_0062d85c + local_420;
    }
    SetRect(local_24, local_38, local_3c, local_34 + local_38, in_ECX[0x1e0]); // DEVIATION: Win32 SetRect
    FUN_005a9780(G.DAT_0067a7a8);
    FUN_00408700(local_24, 10);
    FUN_004bb800(local_24, 1, 1);
    FUN_00408700(local_24, 10);
    iVar1 = FUN_00407f90(local_24);
    local_38 = local_24.left + (iVar1 / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_424 = FUN_0040ef70();
    }
    else {
      local_424 = FUN_0040ef70();
    }
    iVar1 = local_3c + (local_424 / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_2a4 = G.DAT_0069b030;
    }
    else {
      local_2a4 = G.DAT_0069b028;
    }
    uVar5 = 0;
    iVar4 = local_38;
    local_3c = iVar1;
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xc08], local_38, iVar1, 0);
    FUN_005c0f57(local_2a4, uVar3, iVar4, iVar1, uVar5);
    if (in_ECX[0x154] === 0) {
      iVar1 = FUN_0040ef70();
      local_24.top = local_24.top + iVar1;
    }
    else {
      iVar1 = FUN_0040ef70();
      local_24.top = local_24.top + iVar1;
    }
    local_24.bottom = in_ECX[0x1e0] + G.DAT_0062d85c * -2;
    local_24.left = local_24.left + G.DAT_0062d858 * 2;
    local_24.right = local_24.right + G.DAT_0062d858 * -2;
    FUN_00408700(local_24, 10);
    FUN_004bb800(local_24, 1, 1);
    FUN_005c0333(local_24, G.DAT_00635a18);
    local_138 = 0;
    if ((local_30 !== 0) || ((local_28 !== 0 && (param_2 === 1)))) {
      local_138 = 1;
    }
    FUN_00450480(local_24, param_2, local_138);
    break;
  case 9:
    FUN_005c19ad(10);
    local_38 = local_2c;
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_34 = ((iVar1 + G.DAT_0062d858 * -2) / 2 | 0) + G.DAT_0062d858 * -5;
    if (in_ECX[0x154] === 0) {
      local_42c = FUN_0040ef70();
    }
    else {
      local_42c = FUN_0040ef70();
    }
    if (in_ECX[0x154] === 0) {
      local_430 = FUN_0040ef70();
    }
    else {
      local_430 = FUN_0040ef70();
    }
    local_14 = G.DAT_0062d85c * 2 + local_42c * 4 + local_430;
    local_3c = in_ECX[0x144] +
               (((in_ECX[0x1e0] - in_ECX[0x144]) - local_14) / 2 | 0);
    if (local_28 !== 0) {
      FUN_004aef20(G.DAT_00679640);
      if (param_2 === 0) {
        local_434 = FUN_00428b0c(G.DAT_00628420[0xc0c]);
      }
      else {
        local_434 = FUN_00428b0c(G.DAT_00628420[0xc10]);
      }
      FUN_00414d70(local_434);
      if (in_ECX[0x154] === 0) {
        local_2a8 = G.DAT_0069b030;
      }
      else {
        local_2a8 = G.DAT_0069b028;
      }
      FUN_005c0f57(local_2a8, G.DAT_00679640, local_38, in_ECX[0x144] + G.DAT_0062d85c, 5);
    }
    SetRect(local_24, local_38, local_3c, local_34 + local_38, local_14 + local_3c); // DEVIATION: Win32 SetRect
    FUN_005a9780(G.DAT_0067a7a8);
    FUN_00408700(local_24, 10);
    FUN_004bb800(local_24, 1, 1);
    FUN_00408700(local_24, 10);
    iVar1 = FUN_00407f90(local_24);
    local_38 = local_24.left + (iVar1 / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_43c = FUN_0040ef70();
    }
    else {
      local_43c = FUN_0040ef70();
    }
    local_3c = local_3c + G.DAT_0062d85c + (local_43c / 2 | 0);
    FUN_004aef20(G.DAT_00679640);
    if (local_30 === 0) {
      uVar3 = FUN_00428b0c(G.DAT_00628420[0xc18]);
      FUN_00414d70(uVar3);
      FUN_0040fe10();
      FUN_0040ff30(G.DAT_0064c6a2[G.DAT_006d1da0 * 0x594]);
      FUN_0040fe10();
      uVar3 = FUN_00428b0c(G.DAT_00628420[0xc1c]);
      FUN_00414d70(uVar3);
    }
    else {
      uVar3 = FUN_00428b0c(G.DAT_00628420[0xc14]);
      FUN_00414d70(uVar3);
    }
    if (in_ECX[0x154] === 0) {
      local_2ac = G.DAT_0069b030;
    }
    else {
      local_2ac = G.DAT_0069b028;
    }
    FUN_005c0f57(local_2ac, G.DAT_00679640, local_38, local_3c, 0);
    local_24.bottom = local_24.bottom + G.DAT_0062d85c * -2;
    local_24.left = local_24.left + G.DAT_0062d858 * 2;
    local_24.right = local_24.right + G.DAT_0062d858 * -2;
    if (in_ECX[0x154] === 0) {
      local_444 = FUN_0040ef70();
    }
    else {
      local_444 = FUN_0040ef70();
    }
    local_24.top = local_24.top + G.DAT_0062d85c + local_444;
    FUN_00408700(local_24, 10);
    FUN_004bb800(local_24, 1, 1);
    FUN_005c0333(local_24, G.DAT_00635a18);
    FUN_004aef20(G.DAT_00679640);
    FUN_00414d70(s_000000_006324e0);
    if (in_ECX[0x154] === 0) {
      local_448 = FUN_0040efd0(G.DAT_00679640);
    }
    else {
      local_448 = FUN_0040efd0(G.DAT_00679640);
    }
    local_34 = ((local_448 * 3) / 2) | 0;
    FUN_004aef20(G.DAT_00679640);
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xc20]);
    FUN_00414d70(uVar3);
    FUN_0040fe10();
    if (in_ECX[0x154] === 0) {
      local_44c = FUN_0040efd0(G.DAT_00679640);
    }
    else {
      local_44c = FUN_0040efd0(G.DAT_00679640);
    }
    iVar1 = FUN_00407f90(local_24);
    local_38 = (local_24.left + (iVar1 / 2 | 0)) - ((local_34 + local_44c) / 2 | 0);
    iVar1 = FUN_00407fc0(local_24);
    local_3c = local_24.top + (iVar1 / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_2b0 = G.DAT_0069b030;
    }
    else {
      local_2b0 = G.DAT_0069b028;
    }
    FUN_005c0f57(local_2b0, G.DAT_00679640, local_38, local_3c, 1);
    pvVar2 = operator_new(0x48); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_2b8 = 0;
    }
    else {
      local_2b8 = FUN_004187a0();
    }
    in_ECX[0x374 + param_2 * 4] = local_2b8;
    if (in_ECX[0x154] === 0) {
      local_2bc = G.DAT_0069b030;
    }
    else {
      local_2bc = G.DAT_0069b028;
    }
    FUN_005d268e(local_2bc);
    if (in_ECX[0x154] === 0) {
      iVar1 = FUN_0040efd0(G.DAT_00679640);
      local_38 = local_38 + iVar1;
    }
    else {
      iVar1 = FUN_0040efd0(G.DAT_00679640);
      local_38 = local_38 + iVar1;
    }
    if (in_ECX[0x154] === 0) {
      local_14 = FUN_0040ef70();
    }
    else {
      local_14 = FUN_0040ef70();
    }
    local_3c = local_3c - (local_14 / 2 | 0);
    SetRect(local_24, local_38, local_3c, local_34 + local_38, local_14 + local_3c); // DEVIATION: Win32 SetRect
    __itoa(in_ECX[0x22c + param_2 * 4], local_158, 10); // DEVIATION: Win32 __itoa
    if (in_ECX === 0) {
      local_458 = 0;
    }
    else {
      local_458 = in_ECX + 0x48;
    }
    FUN_00418910(local_458, param_2 + 0x3f8, local_24, local_158);
    FUN_004189c0(6);
    FUN_00530eb0(LAB_004020e5);
    FUN_004bb5e0();
    break;
  case 10:
    if (local_28 === 0) {
      in_ECX[0x10420 + param_2 * 4] = 6;
    }
    else {
      in_ECX[0x10420 + param_2 * 4] = 5;
    }
    FUN_005c19ad(10);
    local_38 = local_2c;
    local_3c = in_ECX[0x144] + G.DAT_0062d85c;
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_34 = ((iVar1 + G.DAT_0062d858 * -2) / 2 | 0) + G.DAT_0062d858 * -5;
    if (local_28 !== 0) {
      FUN_004aef20(G.DAT_00679640);
      if (param_2 === 0) {
        local_45c = FUN_00428b0c(G.DAT_00628420[0xc0c]);
      }
      else {
        local_45c = FUN_00428b0c(G.DAT_00628420[0xc10]);
      }
      FUN_00414d70(local_45c);
      if (in_ECX[0x154] === 0) {
        local_2c0 = G.DAT_0069b030;
      }
      else {
        local_2c0 = G.DAT_0069b028;
      }
      FUN_005c0f57(local_2c0, G.DAT_00679640, local_38, local_3c, 5);
      if (in_ECX[0x154] === 0) {
        local_464 = FUN_0040ef70();
      }
      else {
        local_464 = FUN_0040ef70();
      }
      local_3c = local_3c + G.DAT_0062d85c + local_464;
    }
    SetRect(local_24, local_38, local_3c, local_34 + local_38, in_ECX[0x1e0]); // DEVIATION: Win32 SetRect
    FUN_005a9780(G.DAT_0067a7a8);
    FUN_00408700(local_24, 10);
    FUN_004bb800(local_24, 1, 1);
    FUN_00408700(local_24, 10);
    iVar1 = FUN_00407f90(local_24);
    local_38 = local_24.left + (iVar1 / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_468 = FUN_0040ef70();
    }
    else {
      local_468 = FUN_0040ef70();
    }
    iVar1 = local_3c + (local_468 / 2 | 0);
    local_3c = iVar1;
    if (local_28 === 0) {
      if (in_ECX[0x154] === 0) {
        local_2c4 = G.DAT_0069b030;
      }
      else {
        local_2c4 = G.DAT_0069b028;
      }
      uVar5 = 0;
      iVar4 = local_38;
      uVar3 = FUN_00428b0c(G.DAT_00628420[0xc24], local_38, iVar1, 0);
      FUN_005c0f57(local_2c4, uVar3, iVar4, iVar1, uVar5);
    }
    if ((local_28 === 0) && (local_30 === 0)) {
      if (in_ECX[0x154] === 0) {
        local_488 = FUN_0040ef70();
      }
      else {
        local_488 = FUN_0040ef70();
      }
      local_164 = local_24.top + local_488;
      local_168 = (local_34 / 2) | 0;
      if (in_ECX[0x154] === 0) {
        extraout_EAX_01 = FUN_Timevec_dtor(G.DAT_0068abd8);
        local_48c = extraout_EAX_01;
      }
      else {
        extraout_EAX_02 = FUN_Timevec_dtor(G.DAT_0068abe0);
        local_48c = extraout_EAX_02;
      }
      local_16c = local_24.top + G.DAT_0062d868 * 2 + local_470 + local_474;
      local_160 = G.DAT_0062d868 * 2 + local_48c;
      SetRect(local_17c, G.DAT_0062d858 * 2 + local_24.left, local_164, local_24.left + local_168,
              local_164 + local_160); // DEVIATION: Win32 SetRect
    }
    else {
      local_164 = G.DAT_0062d85c * 2 + local_24.top + -1;
      if (in_ECX[0x154] === 0) {
        local_470 = FUN_0040ef70();
      }
      else {
        local_470 = FUN_0040ef70();
      }
      if (in_ECX[0x154] === 0) {
        extraout_EAX = FUN_Timevec_dtor(G.DAT_0068abd8);
        local_474 = extraout_EAX;
      }
      else {
        extraout_EAX_00 = FUN_Timevec_dtor(G.DAT_0068abe0);
        local_474 = extraout_EAX_00;
      }
      local_16c = local_24.top + G.DAT_0062d868 * 2 + local_470 + local_474;
      local_168 = local_34 + G.DAT_0062d858 * -3;
      local_160 = ((local_16c - local_164) / 2) | 0;
      SetRect(local_17c, G.DAT_0062d858 * 2 + local_24.left, local_164, local_24.left + local_168,
              local_164 + local_160); // DEVIATION: Win32 SetRect
      pvVar2 = operator_new(0x3c); // DEVIATION: Win32 operator new
      if (pvVar2 === 0 || pvVar2 === null) {
        local_2cc = 0;
      }
      else {
        local_2cc = FUN_0040f3e0();
      }
      in_ECX[0x39c + param_2 * 4] = local_2cc;
      if (param_2 === 0) {
        local_478 = FUN_00428b0c(G.DAT_00628420[0xdbc]);
      }
      else {
        local_478 = FUN_00428b0c(G.DAT_00628420[0xdc0]);
      }
      if (in_ECX === 0) {
        local_47c = 0;
      }
      else {
        local_47c = in_ECX + 0x48;
      }
      FUN_0040f680(local_47c, param_2 + 0x404, local_17c, local_478);
      FUN_0040f880(LAB_00402496 + ((param_2 === 0 ? 0 : -1) & 0xffffef93));
      pvVar2 = operator_new(0x3c); // DEVIATION: Win32 operator new
      if (pvVar2 === 0 || pvVar2 === null) {
        local_2d4 = 0;
      }
      else {
        local_2d4 = FUN_0040f3e0();
      }
      in_ECX[0x3a4 + param_2 * 4] = local_2d4;
      if (param_2 === 0) {
        local_480 = FUN_00428b0c(G.DAT_00628420[0xdc4]);
      }
      else {
        local_480 = FUN_00428b0c(G.DAT_00628420[0xdc0]);
      }
      if (in_ECX === 0) {
        local_484 = 0;
      }
      else {
        local_484 = in_ECX + 0x48;
      }
      FUN_0040f680(local_484, param_2 + 0x406, local_17c, local_480);
      FUN_0040f880(LAB_004034d1 + ((param_2 === 0 ? 0 : -1) & 0xffffdf58));
      if ((param_2 === 0) && (s8(in_ECX[0x3bd]) !== 0)) {
        FUN_0043c5f0();
        FUN_0040f380();
      }
      else if (param_2 === 0) {
        FUN_0040f380();
        FUN_0043c5f0();
      }
      if ((param_2 === 1) && (s8(in_ECX[0x3bc]) !== 0)) {
        FUN_0043c5f0();
        FUN_0040f380();
        FUN_00453c40();
      }
      else if (param_2 === 1) {
        FUN_0040f380();
        FUN_0043c5f0();
      }
      local_164 = local_164 + local_160;
      local_168 = (local_34 / 2) | 0;
      SetRect(local_17c, G.DAT_0062d858 * 2 + local_24.left, local_164, local_24.left + local_168,
              local_164 + local_160); // DEVIATION: Win32 SetRect
    }
    pvVar2 = operator_new(0x3c); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_2dc = 0;
    }
    else {
      local_2dc = FUN_0040f3e0();
    }
    in_ECX[0x38c + param_2 * 4] = local_2dc;
    if (in_ECX === 0) {
      local_490 = 0;
    }
    else {
      local_490 = in_ECX + 0x48;
    }
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xc28]);
    FUN_0040f680(local_490, param_2 + 0x400, local_17c, uVar3);
    FUN_0040f880(LAB_00401f55 + ((param_2 === 0 ? 0 : -1) & 0xfffff637));
    if ((G.DAT_0067a994 !== 9) &&
       (((param_2 === 0 && (s8(in_ECX[0x3bd]) === 0)) ||
        ((param_2 === 1 && (s8(in_ECX[0x3bc]) === 0)))))) {
      FUN_00453c40();
    }
    SetRect(local_17c, local_17c.right, local_164, local_17c.right + local_168 + G.DAT_0062d858 * -3,
            local_164 + local_160); // DEVIATION: Win32 SetRect
    pvVar2 = operator_new(0x3c); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_2e4 = 0;
    }
    else {
      local_2e4 = FUN_0040f3e0();
    }
    in_ECX[0x394 + param_2 * 4] = local_2e4;
    if (in_ECX === 0) {
      local_494 = 0;
    }
    else {
      local_494 = in_ECX + 0x48;
    }
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xc2c]);
    FUN_0040f680(local_494, param_2 + 0x402, local_17c, uVar3);
    FUN_0040f880(LAB_004033d7 + ((param_2 === 0 ? 0 : -1) & 0xfffff5a1));
    if ((G.DAT_0067a994 !== 9) &&
       (((param_2 === 0 && (s8(in_ECX[0x3bd]) === 0)) ||
        ((param_2 === 1 && (s8(in_ECX[0x3bc]) === 0)))))) {
      FUN_00453c40();
    }
    local_24.top = local_17c.bottom;
    local_24.bottom = in_ECX[0x1e0] + G.DAT_0062d85c * -2;
    local_24.left = local_24.left + G.DAT_0062d858 * 2;
    local_24.right = local_24.right + G.DAT_0062d858 * -2;
    FUN_00408700(local_24, 10);
    FUN_004bb800(local_24, 1, 1);
    FUN_005c0333(local_24, G.DAT_00635a18);
    local_15c = 1;
    if ((local_30 !== 0) || ((local_28 !== 0 && (param_2 === 1)))) {
      local_15c = 0;
    }
    FUN_004683f0(local_24, param_2, local_15c);
    break;
  case 0xb:
    FUN_005c19ad(10);
    if (in_ECX[0x154] === 0) {
      local_498 = FUN_0040ef70();
    }
    else {
      local_498 = FUN_0040ef70();
    }
    local_14 = local_498 * 3 + G.DAT_0062d85c * 2;
    iVar1 = in_ECX[0x144] +
            (((in_ECX[0x1e0] - in_ECX[0x144]) - local_14) / 2 | 0);
    local_38 = local_2c;
    if (in_ECX[0x154] === 0) {
      local_2e8 = G.DAT_0069b030;
    }
    else {
      local_2e8 = G.DAT_0069b028;
    }
    uVar5 = 5;
    iVar4 = local_2c;
    local_3c = iVar1;
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xc30], local_2c, iVar1, 5);
    FUN_005c0f57(local_2e8, uVar3, iVar4, iVar1, uVar5);
    local_184 = FUN_00428b0c(G.DAT_00628420[0xc34]);
    local_180 = FUN_00428b0c(G.DAT_00628420[0xc38]);
    if (in_ECX[0x154] === 0) {
      local_2ec = G.DAT_0069b030;
    }
    else {
      local_2ec = G.DAT_0069b028;
    }
    FUN_005d4167(local_2ec);
    pvVar2 = operator_new(0x50); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_2f4 = 0;
    }
    else {
      local_2f4 = FUN_00531010();
    }
    in_ECX[0x36c + param_2 * 4] = local_2f4;
    local_38 = local_2c;
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_34 = ((iVar1 + G.DAT_0062d858 * -2) / 2) | 0;
    if (in_ECX[0x154] === 0) {
      local_4a4 = FUN_0040ef70();
    }
    else {
      local_4a4 = FUN_0040ef70();
    }
    local_3c = local_3c + G.DAT_0062d85c + local_4a4;
    if (in_ECX[0x154] === 0) {
      local_4a8 = FUN_0040ef70();
    }
    else {
      local_4a8 = FUN_0040ef70();
    }
    local_14 = local_4a8 * 2 + G.DAT_0062d85c;
    SetRect(local_24, local_38, local_3c, local_34 + local_38, local_14 + local_3c); // DEVIATION: Win32 SetRect
    if (in_ECX === 0) {
      local_4ac = 0;
    }
    else {
      local_4ac = in_ECX + 0x48;
    }
    FUN_005310a0(local_4ac, 0x40c, local_24, 1, 2, [local_184, local_180]);
    FUN_004472f0(in_ECX[0x21c]);
    FUN_005311e0(LAB_00401d75);
    FUN_005311b0(LAB_00401c99);
    in_ECX[0x1e4 + param_2 * 4] = 1;
    break;
  case 0xc:
    local_1ac[0] = FUN_00428b0c(G.DAT_00628420[0xc50]);
    local_1ac[1] = FUN_00428b0c(G.DAT_00628420[0xc54]);
    local_1ac[2] = FUN_00428b0c(G.DAT_00628420[0xc58]);
    local_1ac[3] = FUN_00428b0c(G.DAT_00628420[0xd0c]);
    if (local_28 === 0) {
      if (local_30 === 0) {
        local_1b0 = 0;
      }
      else {
        local_1b0 = 3;
      }
    }
    else if (param_2 === 0) {
      local_1b0 = 1;
    }
    else {
      local_1b0 = 2;
    }
    in_ECX[0x10420 + param_2 * 4] = 5;
    FUN_005c19ad(10);
    local_38 = local_2c;
    local_3c = in_ECX[0x144] + G.DAT_0062d85c;
    if (local_28 !== 0) {
      FUN_004aef20(G.DAT_00679640);
      if (param_2 === 0) {
        local_4d8 = FUN_00428b0c(G.DAT_00628420[0xc0c]);
      }
      else {
        local_4d8 = FUN_00428b0c(G.DAT_00628420[0xc10]);
      }
      FUN_00414d70(local_4d8);
      if (in_ECX[0x154] === 0) {
        local_30c = G.DAT_0069b030;
      }
      else {
        local_30c = G.DAT_0069b028;
      }
      FUN_005c0f57(local_30c, G.DAT_00679640, local_38, local_3c, 5);
      if (in_ECX[0x154] === 0) {
        local_4e0 = FUN_0040ef70();
      }
      else {
        local_4e0 = FUN_0040ef70();
      }
      local_3c = local_3c + G.DAT_0062d85c + local_4e0;
    }
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_34 = ((iVar1 + G.DAT_0062d858 * -2) / 2 | 0) + G.DAT_0062d858 * -5;
    if (in_ECX[0x154] === 0) {
      local_4e4 = FUN_0040ef70();
    }
    else {
      local_4e4 = FUN_0040ef70();
    }
    if (in_ECX[0x154] === 0) {
      local_4e8 = FUN_0040ef70();
    }
    else {
      local_4e8 = FUN_0040ef70();
    }
    local_14 = in_ECX[0x10420 + param_2 * 4] * local_4e4 +
               in_ECX[0x10420 + param_2 * 4] * G.DAT_0062d85c + G.DAT_0062d85c * 4 +
               local_4e8;
    local_3c = local_3c + (((in_ECX[0x1e0] - local_3c) - local_14) / 2 | 0);
    SetRect(local_24, local_38, local_3c, local_34 + local_38, local_14 + local_3c); // DEVIATION: Win32 SetRect
    FUN_005a9780(G.DAT_0067a7a8);
    FUN_00408700(local_24, 10);
    FUN_004bb800(local_24, 1, 1);
    FUN_00408700(local_24, 10);
    iVar1 = FUN_00407f90(local_24);
    local_38 = local_24.left + (iVar1 / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_4ec = FUN_0040ef70();
    }
    else {
      local_4ec = FUN_0040ef70();
    }
    local_3c = local_3c + G.DAT_0062d85c + (local_4ec / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_310 = G.DAT_0069b030;
    }
    else {
      local_310 = G.DAT_0069b028;
    }
    FUN_005c0f57(local_310, local_1ac[local_1b0], local_38, local_3c, 0);
    if (in_ECX[0x154] === 0) {
      local_4f4 = FUN_0040ef70();
    }
    else {
      local_4f4 = FUN_0040ef70();
    }
    local_24.top = local_24.top + G.DAT_0062d85c + local_4f4;
    if (in_ECX[0x154] === 0) {
      local_4f8 = FUN_0040ef70();
    }
    else {
      local_4f8 = FUN_0040ef70();
    }
    local_24.bottom =
         in_ECX[0x10420 + param_2 * 4] * local_4f8 +
         in_ECX[0x10420 + param_2 * 4] * G.DAT_0062d85c + local_24.top + 2;
    local_24.left = local_24.left + G.DAT_0062d858 * 2;
    local_24.right = local_24.right + G.DAT_0062d858 * -2;
    FUN_00408700(local_24, 10);
    FUN_004bb800(local_24, 1, 1);
    FUN_005c0333(local_24, G.DAT_00635a18);
    FUN_004af3e0(local_24, param_2);
    break;
  case 0xd:
    local_198[0] = FUN_00428b0c(G.DAT_00628420[0xc3c]);
    local_198[1] = FUN_00428b0c(G.DAT_00628420[0xc40]);
    local_19c = 0;
    local_198[2] = FUN_00428b0c(G.DAT_00628420[0xc44]);
    local_198[3] = FUN_00428b0c(G.DAT_00628420[0xc48]);
    local_188 = FUN_00428b0c(G.DAT_00628420[0xc4c]);
    FUN_005c19ad(10);
    local_38 = local_2c;
    local_3c = in_ECX[0x144] + G.DAT_0062d85c;
    if (local_28 !== 0) {
      local_19c = (param_2 !== 0) ? 1 : 0;
      FUN_004aef20(G.DAT_00679640);
      if (param_2 === 0) {
        local_4b0 = FUN_00428b0c(G.DAT_00628420[0xc0c]);
      }
      else {
        local_4b0 = FUN_00428b0c(G.DAT_00628420[0xc10]);
      }
      FUN_00414d70(local_4b0);
      if (in_ECX[0x154] === 0) {
        local_2f8 = G.DAT_0069b030;
      }
      else {
        local_2f8 = G.DAT_0069b028;
      }
      FUN_005c0f57(local_2f8, G.DAT_00679640, local_38, local_3c, 5);
      if (in_ECX[0x154] === 0) {
        local_4b8 = FUN_0040ef70();
      }
      else {
        local_4b8 = FUN_0040ef70();
      }
      local_3c = local_3c + G.DAT_0062d85c + local_4b8;
    }
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_34 = ((iVar1 + G.DAT_0062d858 * -2) / 2 | 0) + G.DAT_0062d858 * -5;
    if (in_ECX[0x154] === 0) {
      local_4bc = FUN_0040ef70();
    }
    else {
      local_4bc = FUN_0040ef70();
    }
    if (in_ECX[0x154] === 0) {
      local_4c0 = FUN_0040ef70();
    }
    else {
      local_4c0 = FUN_0040ef70();
    }
    local_14 = local_4bc * 3 + G.DAT_0062d85c * 9 + local_4c0;
    local_3c = local_3c + (((in_ECX[0x1e0] - local_3c) - local_14) / 2 | 0);
    SetRect(local_24, local_38, local_3c, local_34 + local_38, local_14 + local_3c); // DEVIATION: Win32 SetRect
    FUN_005a9780(G.DAT_0067a7a8);
    FUN_00408700(local_24, 10);
    FUN_004bb800(local_24, 1, 1);
    FUN_00408700(local_24, 10);
    iVar1 = FUN_00407f90(local_24);
    local_38 = local_24.left + (iVar1 / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_4c4 = FUN_0040ef70();
    }
    else {
      local_4c4 = FUN_0040ef70();
    }
    local_3c = local_3c + G.DAT_0062d85c + (local_4c4 / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_2fc = G.DAT_0069b030;
    }
    else {
      local_2fc = G.DAT_0069b028;
    }
    FUN_005c0f57(local_2fc, local_198[local_19c], local_38, local_3c, 0);
    if (in_ECX[0x154] === 0) {
      local_4cc = FUN_0040ef70();
    }
    else {
      local_4cc = FUN_0040ef70();
    }
    local_24.top = local_24.top + G.DAT_0062d85c + local_4cc;
    local_24.bottom = local_24.bottom + G.DAT_0062d85c * -2;
    local_24.left = local_24.left + G.DAT_0062d858 * 2;
    local_24.right = local_24.right + G.DAT_0062d858 * -2;
    FUN_00408700(local_24, 10);
    FUN_004bb800(local_24, 1, 1);
    FUN_005c0333(local_24, G.DAT_00635a18);
    if (in_ECX[0x154] === 0) {
      local_300 = G.DAT_0069b030;
    }
    else {
      local_300 = G.DAT_0069b028;
    }
    FUN_005d4167(local_300);
    pvVar2 = operator_new(0x50); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_308 = 0;
    }
    else {
      local_308 = FUN_00531010();
    }
    in_ECX[0x36c + param_2 * 4] = local_308;
    if (in_ECX === 0) {
      local_4d4 = 0;
    }
    else {
      local_4d4 = in_ECX + 0x48;
    }
    FUN_005310a0(local_4d4, param_2 + 0x424, local_24, 1, 3, [local_198[2], local_198[3], local_188]);
    FUN_004472f0(in_ECX[0x224 + param_2 * 4]);
    FUN_005311e0(LAB_00401d75);
    FUN_005311b0(LAB_00401bd6);
    in_ECX[0x1e4 + param_2 * 4] = 1;
    break;
  case 0xf:
    local_1cc = FUN_00428b0c(G.DAT_00628420[0xbac]);
    local_1c8 = FUN_00428b0c(G.DAT_00628420[0xbb0]);
    local_1c4 = FUN_00428b0c(G.DAT_00628420[0xbb4]);
    local_1c0 = FUN_00428b0c(G.DAT_00628420[3000]);
    local_1bc = FUN_00428b0c(G.DAT_00628420[0xbbc]);
    local_1b8 = FUN_00428b0c(G.DAT_00628420[0xbc4]);
    local_1b4 = FUN_00428b0c(G.DAT_00628420[0xd08]);
    FUN_005c19ad(10);
    local_38 = local_2c;
    if (in_ECX[0x154] === 0) {
      local_4fc = FUN_0040ef70();
    }
    else {
      local_4fc = FUN_0040ef70();
    }
    local_14 = G.DAT_0062d85c * 7 + local_4fc * 8;
    iVar1 = in_ECX[0x144] +
            (((in_ECX[0x1e0] - in_ECX[0x144]) - local_14) / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_314 = G.DAT_0069b030;
    }
    else {
      local_314 = G.DAT_0069b028;
    }
    uVar5 = 5;
    iVar4 = local_38;
    local_3c = iVar1;
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xc5c], local_38, iVar1, 5);
    FUN_005c0f57(local_314, uVar3, iVar4, iVar1, uVar5);
    if (in_ECX[0x154] === 0) {
      local_504 = FUN_0040ef70();
    }
    else {
      local_504 = FUN_0040ef70();
    }
    iVar1 = local_3c + G.DAT_0062d85c * 2 + local_504;
    if (in_ECX[0x154] === 0) {
      local_318 = G.DAT_0069b030;
    }
    else {
      local_318 = G.DAT_0069b028;
    }
    uVar5 = 5;
    iVar4 = local_38;
    local_3c = iVar1;
    uVar3 = FUN_00428b0c(G.DAT_00628420[0xc60], local_38, iVar1, 5);
    FUN_005c0f57(local_318, uVar3, iVar4, iVar1, uVar5);
    if (in_ECX[0x154] === 0) {
      local_50c = FUN_0040ef70();
    }
    else {
      local_50c = FUN_0040ef70();
    }
    local_3c = local_3c + G.DAT_0062d85c + local_50c;
    if (in_ECX[0x154] === 0) {
      local_31c = G.DAT_0069b030;
    }
    else {
      local_31c = G.DAT_0069b028;
    }
    FUN_005d4167(local_31c);
    pvVar2 = operator_new(0x50); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_324 = 0;
    }
    else {
      local_324 = FUN_00531010();
    }
    in_ECX[0x36c + param_2 * 4] = local_324;
    local_38 = local_2c;
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_34 = ((iVar1 + G.DAT_0062d858 * -2) / 2) | 0;
    if (in_ECX[0x154] === 0) {
      local_514 = FUN_0040ef70();
    }
    else {
      local_514 = FUN_0040ef70();
    }
    local_14 = G.DAT_0062d85c * 5 + local_514 * 6;
    SetRect(local_24, local_38, local_3c, local_34 + local_38, local_14 + local_3c); // DEVIATION: Win32 SetRect
    if (in_ECX === 0) {
      local_518 = 0;
    }
    else {
      local_518 = in_ECX + 0x48;
    }
    FUN_005310a0(local_518, 0x41f, local_24, 1, 7, [local_1cc, local_1c8, local_1c4, local_1c0, local_1bc, local_1b8, local_1b4]);
    FUN_004472f0(in_ECX[0x214]);
    FUN_005311e0(LAB_00401d75);
    FUN_005311b0(LAB_00403a30);
    in_ECX[0x1e4 + param_2 * 4] = 1;
    break;
  case 0x11:
    if (local_28 === 0) {
      in_ECX[0x10420 + param_2 * 4] = 7;
    }
    else {
      in_ECX[0x10420 + param_2 * 4] = 6;
    }
    FUN_005c19ad(10);
    local_38 = local_2c;
    local_3c = in_ECX[0x144] + G.DAT_0062d85c;
    iVar1 = FUN_00407f90(in_ECX[0x140]);
    local_34 = ((iVar1 + G.DAT_0062d858 * -2) / 2 | 0) + G.DAT_0062d858 * -5;
    if (local_28 !== 0) {
      FUN_004aef20(G.DAT_00679640);
      if (param_2 === 0) {
        local_51c = FUN_00428b0c(G.DAT_00628420[0xc0c]);
      }
      else {
        local_51c = FUN_00428b0c(G.DAT_00628420[0xc10]);
      }
      FUN_00414d70(local_51c);
      if (in_ECX[0x154] === 0) {
        local_328 = G.DAT_0069b030;
      }
      else {
        local_328 = G.DAT_0069b028;
      }
      FUN_005c0f57(local_328, G.DAT_00679640, local_38, local_3c, 5);
      if (in_ECX[0x154] === 0) {
        local_524 = FUN_0040ef70();
      }
      else {
        local_524 = FUN_0040ef70();
      }
      local_3c = local_3c + G.DAT_0062d85c + local_524;
    }
    SetRect(local_24, local_38, local_3c, local_34 + local_38, in_ECX[0x1e0]); // DEVIATION: Win32 SetRect
    FUN_005a9780(G.DAT_0067a7a8);
    FUN_00408700(local_24, 10);
    FUN_004bb800(local_24, 1, 1);
    FUN_00408700(local_24, 10);
    iVar1 = FUN_00407f90(local_24);
    local_38 = local_24.left + (iVar1 / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_528 = FUN_0040ef70();
    }
    else {
      local_528 = FUN_0040ef70();
    }
    local_3c = local_3c + (local_528 / 2 | 0);
    if ((local_28 === 0) && (local_30 === 0)) {
      local_1d8 = FUN_00428b0c(G.DAT_00628420[0xc68]);
      if (in_ECX[0x154] === 0) {
        local_33c = G.DAT_0069b030;
      }
      else {
        local_33c = G.DAT_0069b028;
      }
      FUN_005c0f57(local_33c, local_1d8, local_38, local_3c, 0);
    }
    else {
      local_1dc = G.DAT_0062d85c * 2 + local_24.top + -1;
      local_1e0 = local_34 + G.DAT_0062d858 * -3;
      if (in_ECX[0x154] === 0) {
        local_52c = FUN_0040ef70();
      }
      else {
        local_52c = FUN_0040ef70();
      }
      local_1d4 = local_52c - (G.DAT_0062d85c * 2 + -1);
      SetRect(local_1f0, G.DAT_0062d858 * 2 + local_24.left, local_1dc, local_24.left + local_1e0,
              local_1d4 + local_1dc); // DEVIATION: Win32 SetRect
      pvVar2 = operator_new(0x3c); // DEVIATION: Win32 operator new
      if (pvVar2 === 0 || pvVar2 === null) {
        local_330 = 0;
      }
      else {
        local_330 = FUN_0040f3e0();
      }
      in_ECX[0x39c + param_2 * 4] = local_330;
      if (param_2 === 0) {
        local_530 = FUN_00428b0c(G.DAT_00628420[0xdc8]);
      }
      else {
        local_530 = FUN_00428b0c(G.DAT_00628420[0xdcc]);
      }
      if (in_ECX === 0) {
        local_534 = 0;
      }
      else {
        local_534 = in_ECX + 0x48;
      }
      FUN_0040f680(local_534, param_2 + 0x408, local_1f0, local_530);
      FUN_0040f880(LAB_004038d7 + ((param_2 === 0 ? 0 : -1) & 0xffffdd6e));
      pvVar2 = operator_new(0x3c); // DEVIATION: Win32 operator new
      if (pvVar2 === 0 || pvVar2 === null) {
        local_338 = 0;
      }
      else {
        local_338 = FUN_0040f3e0();
      }
      in_ECX[0x3a4 + param_2 * 4] = local_338;
      if (param_2 === 0) {
        local_538 = FUN_00428b0c(G.DAT_00628420[0xdd0]);
      }
      else {
        local_538 = FUN_00428b0c(G.DAT_00628420[0xdcc]);
      }
      if (in_ECX === 0) {
        local_53c = 0;
      }
      else {
        local_53c = in_ECX + 0x48;
      }
      FUN_0040f680(local_53c, param_2 + 0x40a, local_1f0, local_538);
      FUN_0040f880(LAB_00401979 + ((param_2 === 0 ? 0 : -1) & 0xfffffccc));
      if ((param_2 === 0) && (s8(in_ECX[0x3be]) !== 0)) {
        FUN_0043c5f0();
        FUN_0040f380();
      }
      else if (param_2 === 0) {
        FUN_0040f380();
        FUN_0043c5f0();
      }
      if ((param_2 === 1) && (s8(in_ECX[0x3bf]) !== 0)) {
        FUN_0043c5f0();
        FUN_0040f380();
        FUN_00453c40();
      }
      else if (param_2 === 1) {
        FUN_0040f380();
        FUN_0043c5f0();
      }
    }
    if (in_ECX[0x154] === 0) {
      iVar1 = FUN_0040ef70();
      local_24.top = local_24.top + iVar1;
    }
    else {
      iVar1 = FUN_0040ef70();
      local_24.top = local_24.top + iVar1;
    }
    local_24.bottom = in_ECX[0x1e0] + G.DAT_0062d85c * -2;
    local_24.left = local_24.left + G.DAT_0062d858 * 2;
    local_24.right = local_24.right + G.DAT_0062d858 * -2;
    FUN_00408700(local_24, 10);
    FUN_004bb800(local_24, 1, 1);
    FUN_005c0333(local_24, G.DAT_00635a18);
    local_1d0 = 1;
    if ((local_30 !== 0) || ((local_28 !== 0 && (param_2 === 1)))) {
      local_1d0 = 0;
    }
    FUN_00587a90(local_24, param_2, local_1d0);
    break;
  case 0x12:
    uVar3 = FUN_00410070(in_ECX[0x118]);
    FUN_00421d60(0, uVar3);
    FUN_0040bbb0();
    FUN_0040bc10(0x2e2);
    FUN_00426ff0(G.DAT_00679640, local_9c);
    FUN_005c19ad(10);
    if (in_ECX[0x154] === 0) {
      local_a0 = FUN_0040efd0(local_9c);
    }
    else {
      local_a0 = FUN_0040efd0(local_9c);
    }
    local_38 = (in_ECX[0x140] + ((in_ECX[0x148] - in_ECX[0x140]) / 2 | 0)) -
               (local_a0 / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_354 = FUN_0040ef70();
    }
    else {
      local_354 = FUN_0040ef70();
    }
    local_3c = (in_ECX[0x144] + ((in_ECX[0x1e0] - in_ECX[0x144]) / 2 | 0)
               + G.DAT_0062d85c) - (local_354 / 2 | 0);
    if (in_ECX[0x154] === 0) {
      local_20c = G.DAT_0069b020;
    }
    else {
      local_20c = G.DAT_0069b018;
    }
    FUN_005c0f57(local_20c, local_9c, local_38, local_3c, 5);
    FUN_0043c5f0();
    FUN_00453c40();
    FUN_0040f380();
    FUN_00453c40();
    FUN_00453c40();
    if ((s8(in_ECX[0x3bd]) === 0) && (s8(in_ECX[0x3be]) === 0)) {
      FUN_0043c5f0();
      FUN_00453c40();
      FUN_0040f380();
      FUN_00453c80();
    }
    else {
      FUN_0040f380();
      FUN_00453c80();
      FUN_0043c5f0();
      FUN_00453c40();
    }
    break;
  case 0x13:
    FUN_004dbee6(in_ECX[0x234], 0);
    pvVar2 = operator_new(0x48); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_1f8 = 0;
    }
    else {
      local_1f8 = FUN_004187a0();
    }
    in_ECX[0x294] = local_1f8;
    local_38 = in_ECX[0x140] + G.DAT_0062d858;
    local_34 = FUN_00407f90(in_ECX[0x140]);
    local_34 = local_34 + G.DAT_0062d858 * -2;
    local_3c = in_ECX[0x144] + G.DAT_0062d85c;
    local_14 = in_ECX[0x1e0] - local_3c;
    SetRect(local_24, local_38, local_3c, local_34 + local_38, local_14 + local_3c); // DEVIATION: Win32 SetRect
    if (in_ECX[0x154] === 0) {
      local_1fc = G.DAT_0067a7a0;
    }
    else {
      local_1fc = G.DAT_0067a798;
    }
    FUN_005d268e(local_1fc);
    if (in_ECX === 0) {
      local_344 = 0;
    }
    else {
      local_344 = in_ECX + 0x48;
    }
    FUN_004bb620(local_344, 0x426, local_24, G.DAT_006a5b58, 0x122, 0);
    FUN_004189c0(0x2000);
    FUN_00418a00(LAB_00402040);
    FUN_0040f380();
    break;
  case 0x14:
    FUN_004085f0();
    FUN_004dbee6(in_ECX[0x234], 1);
    pvVar2 = operator_new(0x48); // DEVIATION: Win32 operator new
    if (pvVar2 === 0 || pvVar2 === null) {
      local_204 = 0;
    }
    else {
      local_204 = FUN_004187a0();
    }
    in_ECX[0x294] = local_204;
    local_38 = in_ECX[0x140] + G.DAT_0062d858;
    local_34 = FUN_00407f90(in_ECX[0x140]);
    local_34 = local_34 + G.DAT_0062d858 * -2;
    local_3c = in_ECX[0x144] + G.DAT_0062d85c;
    local_14 = in_ECX[0x1e0] - local_3c;
    SetRect(local_24, local_38, local_3c, local_34 + local_38, local_14 + local_3c); // DEVIATION: Win32 SetRect
    if (in_ECX[0x154] === 0) {
      local_208 = G.DAT_0067a7a0;
    }
    else {
      local_208 = G.DAT_0067a798;
    }
    FUN_005d268e(local_208);
    if (in_ECX === 0) {
      local_34c = 0;
    }
    else {
      local_34c = in_ECX + 0x48;
    }
    FUN_004bb620(local_34c, 0x426, local_24, G.DAT_006a5b58, 0x122, 0);
    FUN_004189c0(0x2000);
    FUN_00418a00(LAB_00402040);
    FUN_0040f380();
    _DAT_0067a9e0 = 1;
    if (in_ECX[0x234] !== 0) {
      iVar1 = in_ECX[0x234];
      local_4c = iVar1 + 0x20;
      if (((iVar1[0x10] === 0xf) && (local_4c[0] === 8)) ||
         ((iVar1[0x10] === 0xe && (local_4c[0] === 8)))) {
        local_40 = iVar1[0x28];
        local_48 = iVar1 + 0x2c;
        for (local_44 = 0; local_44 < local_40; local_44 = local_44 + 1) {
          uVar3 = local_48[0];
          local_48 = local_48 + 1;
          iVar1 = FUN_004bd9f0(G.DAT_006d1da0, uVar3);
          if (iVar1 === 0) {
            in_ECX[0x238] = 0;
          }
        }
      }
    }
    if (in_ECX[0x238] === 0) {
      FUN_00453c40();
    }
    else {
      FUN_00453c80();
    }
    break;
  }
  FUN_0052e971(in_ECX);
  FUN_00408490(in_ECX[0x3ac]);
  return;
}



// ============================================================
// Function: FUN_0052d4d2 @ 0x0052D4D2
// Size: 81 bytes
// ============================================================

// parley_on_treaty_status_select
export function FUN_0052d4d2(param_1, param_2) {
  let local_8;

  local_8 = FUN_005c62ee();
  if (local_8 === 0) {
    local_8 = 0;
  }
  else {
    local_8 = local_8 + -0x48;
  }
  FUN_004518d0();
  local_8[0x218] = param_2;
  return;
}



// ============================================================
// Function: FUN_0052d523 @ 0x0052D523
// Size: 101 bytes
// ============================================================

// parley_on_demand_type_select
export function FUN_0052d523(param_1, param_2) {
  let local_8;

  local_8 = FUN_005c62ee();
  if (local_8 === 0) {
    local_8 = 0;
  }
  else {
    local_8 = local_8 + -0x48;
  }
  FUN_004518d0();
  local_8[0x214] = param_2;
  FUN_00526ca0(local_8[500], 1);
  return;
}



// ============================================================
// Function: FUN_0052d588 @ 0x0052D588
// Size: 85 bytes
// ============================================================

// parley_on_generic_select
export function FUN_0052d588(param_1, param_2) {
  let local_8;

  local_8 = FUN_005c62ee();
  if (local_8 === 0) {
    local_8 = 0;
  }
  else {
    local_8 = local_8 + -0x48;
  }
  FUN_004518d0();
  local_8[-0xe6c + param_1 * 4] = param_2;
  return;
}



// ============================================================
// Function: FUN_0052d5dd @ 0x0052D5DD
// Size: 265 bytes
// ============================================================

// parley_on_tribute_type_select
export function FUN_0052d5dd(param_1, param_2) {
  let local_8;

  local_8 = FUN_005c62ee();
  if (local_8 === 0) {
    local_8 = 0;
  }
  else {
    local_8 = local_8 + -0x48;
  }
  FUN_004518d0();
  local_8[0x220] = param_2;
  switch (param_2) {
  case 0:
    local_8[0x1ec] = 7;
    break;
  case 1:
    local_8[0x1ec] = 8;
    break;
  case 2:
    local_8[0x1ec] = 9;
    break;
  case 3:
    local_8[0x1ec] = 10;
    break;
  case 4:
    local_8[0x1ec] = 0xb;
    break;
  }
  local_8[0x1fc] = G.DAT_0062d7d4[local_8[0x1ec] * 2];
  local_8[500] = local_8[0x1fc];
  FUN_00526ca0(local_8[500], 1);
  return;
}



// ============================================================
// Function: FUN_0052d6ff @ 0x0052D6FF
// Size: 216 bytes
// ============================================================

// parley_on_war_peace_select
export function FUN_0052d6ff(param_1, param_2) {
  let local_8;

  local_8 = FUN_005c62ee();
  if (local_8 === 0) {
    local_8 = 0;
  }
  else {
    local_8 = local_8 + -0x48;
  }
  FUN_004518d0();
  local_8[0x21c] = param_2;
  if (param_2 === 0) {
    local_8[0x1ec] = 0xc;
  }
  else if (param_2 === 1) {
    local_8[0x1ec] = 0xd;
  }
  local_8[0x1fc] = G.DAT_0062d7d4[local_8[0x1ec] * 2];
  local_8[500] = local_8[0x1fc];
  FUN_00526ca0(local_8[500], 1);
  return;
}



// ============================================================
// Function: FUN_0052d7dc @ 0x0052D7DC
// Size: 425 bytes
// ============================================================

// parley_on_propose_item_toggle
export function FUN_0052d7dc(param_1, param_2) {
  let uVar1;
  let local_10;
  let local_8;

  local_10 = FUN_005c62ee();
  if (local_10 === 0) {
    local_10 = 0;
  }
  else {
    local_10 = local_10 + -0x48;
  }
  FUN_004518d0();
  local_8 = (param_1 === 0x3f4) ? 1 : 0;
  switch (param_2) {
  case 0:
    uVar1 = CCheckListBox_GetCheckStyle(local_10[0x36c + local_8 * 4]);
    FUN_00447210(0);
    FUN_00421ca0(6);
    FUN_00421ca0(6);
    if (uVar1 === 0) {
      FUN_004472f0(1);
    }
    break;
  case 1:
  case 2:
  case 3:
  case 4:
  case 5:
    FUN_00421ca0(0);
    FUN_00421ca0(0);
    FUN_00421ca0(6);
    FUN_00421ca0(6);
    break;
  case 6:
    uVar1 = CCheckListBox_GetCheckStyle(local_10[0x36c + local_8 * 4]);
    FUN_00421ca0(0);
    FUN_00421ca0(0);
    FUN_00447210(6);
    if (uVar1 === 6) {
      FUN_004472f0(0);
    }
    break;
  }
  return;
}



// ============================================================
// Function: FUN_0052d9a1 @ 0x0052D9A1
// Size: 130 bytes
// ============================================================

// parley_validate_and_advance
export function FUN_0052d9a1(param_1) {
  let pcVar1;
  let local_8;

  local_8 = FUN_005c62ee();
  if (local_8 === 0) {
    local_8 = 0;
  }
  else {
    local_8 = local_8 + -0x48;
  }
  pcVar1 = streambuf_egptr(local_8[0x348]);
  if ((pcVar1 !== null && pcVar1 !== 0) ||
     (pcVar1 = streambuf_egptr(local_8[0x34c]), pcVar1 !== null && pcVar1 !== 0)) {
    FUN_004518d0();
    FUN_0052dd73(param_1);
  }
  return;
}



// ============================================================
// Function: FUN_0052da23 @ 0x0052DA23
// Size: 282 bytes
// ============================================================

// parley_reject_proposals
export function FUN_0052da23() {
  let local_8;

  local_8 = FUN_005c62ee();
  if (local_8 === 0) {
    local_8 = 0;
  }
  else {
    local_8 = local_8 + -0x48;
  }
  local_8[0x3bd] = 0;
  local_8[0x3be] = 0;
  FUN_0046b14d(0xa6,
               G.DAT_006ad30c[G.DAT_006ad558[local_8[0x118] * 4] * 0x54],
               G.DAT_006d1da0, s8(local_8[0x3be]) | 0, 0, 0, 0, 0, 0, 0);
  FUN_0046b14d(0xa8,
               G.DAT_006ad30c[G.DAT_006ad558[local_8[0x118] * 4] * 0x54],
               G.DAT_006d1da0, s8(local_8[0x3bd]) | 0, 0, 0, 0, 0, 0, 0);
  FUN_0043c5f0();
  FUN_00453c40();
  FUN_0040f380();
  FUN_00453c80();
  return;
}



// ============================================================
// Function: FUN_0052db3d @ 0x0052DB3D
// Size: 321 bytes
// ============================================================

// parley_accept_proposals
export function FUN_0052db3d() {
  let local_8;

  FUN_005c62ee();
  local_8 = FUN_005c62ee();
  if (local_8 === 0) {
    local_8 = 0;
  }
  else {
    local_8 = local_8 + -0x48;
  }
  local_8[0x3bd] = 1;
  local_8[0x3be] = 1;
  FUN_0046b14d(0xa5,
               G.DAT_006ad30c[G.DAT_006ad558[local_8[0x118] * 4] * 0x54],
               G.DAT_006d1da0, s8(local_8[0x3be]) | 0, 0, 0, 0, 0, 0, 0);
  FUN_0046b14d(0xa7,
               G.DAT_006ad30c[G.DAT_006ad558[local_8[0x118] * 4] * 0x54],
               G.DAT_006d1da0, s8(local_8[0x3bd]) | 0, 0, 0, 0, 0, 0, 0);
  FUN_0040f380();
  FUN_00453c80();
  FUN_0043c5f0();
  FUN_00453c40();
  return;
}



// ============================================================
// Function: FUN_0052dc7e @ 0x0052DC7E
// Size: 245 bytes
// ============================================================

// parley_show_intelligence
export function FUN_0052dc7e() {
  let bVar1;
  let iVar2;
  let local_8;

  local_8 = FUN_005c62ee();
  if (local_8 === 0) {
    local_8 = 0;
  }
  else {
    local_8 = local_8 + -0x48;
  }
  iVar2 = FUN_00453e51(G.DAT_006d1da0, 0x18);
  // Original goto LAB_0052dcf1 restructured with flag variable
  bVar1 = false;
  if (iVar2 === 0) {
    iVar2 = FUN_00453e51(G.DAT_006d1da0, 9);
    if (iVar2 === 0) {
      bVar1 = false;
    } else {
      bVar1 = true;
    }
  } else {
    bVar1 = true;
  }
  // LAB_0052dcf1
  if ((((G.DAT_0064c6c0[local_8[0x118] * 4 + G.DAT_006d1da0 * 0x594] & 0x80) !== 0) ||
      (bVar1)) || (G.DAT_00655b07 !== 0)) {
    FUN_0043060b(G.DAT_006d1da0, local_8[0x118]);
  }
  else {
    FUN_00410030(s_NOINTEL_006324e8, G.DAT_0063fc58, 0);
  }
  return;
}



// ============================================================
// Function: FUN_0052dd73 @ 0x0052DD73
// Size: 1425 bytes
// ============================================================

// parley_advance_negotiation
export function FUN_0052dd73() {
  let uVar1;
  let uVar2;
  let local_8;

  local_8 = FUN_005c62ee();
  if (local_8 === 0) {
    local_8 = 0;
  }
  else {
    local_8 = local_8 + -0x48;
  }
  FUN_004518d0();
  switch (local_8[0x1ec]) {
  case 3:
    uVar1 = CCheckListBox_GetCheckStyle(local_8[0x370]);
    local_8[0x208] = uVar1;
    if (local_8[0x208] === 0) {
      local_8[0x1ec] = 6;
      FUN_00453c80();
    }
    else if (local_8[0x208] === 1) {
      local_8[0x1ec] = 0xc;
      if (local_8[0x21c] !== 0) {
        local_8[0x1ec] = 0xd;
      }
      FUN_00453c80();
    }
    else if (local_8[0x208] === 2) {
      if (local_8[0x220] === 0) {
        local_8[0x1ec] = 7;
      }
      else if (local_8[0x220] === 1) {
        local_8[0x1ec] = 8;
      }
      else if (local_8[0x220] === 2) {
        local_8[0x1ec] = 9;
      }
      else if (local_8[0x220] === 3) {
        local_8[0x1ec] = 10;
      }
      else {
        local_8[0x1ec] = 0xb;
      }
      FUN_00453c80();
    }
    else if (local_8[0x208] === 3) {
      local_8[0x1ec] = 5;
      FUN_00453c80();
    }
    else if (local_8[0x208] === 4) {
      local_8[0x1ec] = 0xf;
      FUN_00453c80();
    }
    local_8[0x1f8] = G.DAT_0062d7d0[local_8[0x1ec] * 2];
    local_8[0x1f0] = local_8[0x1f8];
    local_8[0x1fc] = G.DAT_0062d7d4[local_8[0x1ec] * 2];
    local_8[500] = local_8[0x1fc];
    FUN_00526ca0(local_8[0x1f0], 0);
    FUN_00526ca0(local_8[500], 1);
    break;
  case 5:
    uVar1 = CCheckListBox_GetCheckStyle(local_8[0x36c]);
    local_8[0x20c] = uVar1;
    uVar1 = CCheckListBox_GetCheckStyle(local_8[0x370]);
    local_8[0x210] = uVar1;
    local_8[0x1ec] = 0xe;
    local_8[0x1f8] = G.DAT_0062d7d0[local_8[0x1ec] * 2];
    local_8[0x1f0] = local_8[0x1f8];
    local_8[0x1fc] = G.DAT_0062d7d4[local_8[0x1ec] * 2];
    local_8[500] = local_8[0x1fc];
    FUN_00526ca0(local_8[0x1f0], 0);
    FUN_00526ca0(local_8[500], 1);
    break;
  case 6:
  case 7:
  case 8:
  case 9:
  case 10:
  case 0xb:
  case 0xc:
  case 0xd:
  case 0xe:
  case 0xf:
    if (local_8[0x234] !== 0) {
      // operator_delete(local_8[0x234])
      local_8[0x234] = 0;
    }
    uVar2 = FUN_004db690(local_8[0x1ec]);
    local_8[0x234] = uVar2;
    if ((local_8[0x1ec] === 6) && (local_8[0x218] === 3)) {
      if ((G.DAT_0064c6c0[local_8[0x118] * 4 + G.DAT_006d1da0 * 0x594] & 8) === 0) {
        if ((G.DAT_0064c6c0[local_8[0x118] * 4 + G.DAT_006d1da0 * 0x594] & 4) === 0) {
          if ((G.DAT_0064c6c0[local_8[0x118] * 4 + G.DAT_006d1da0 * 0x594] & 2) !== 0) {
            FUN_00467750(G.DAT_006d1da0, local_8[0x118], 2);
          }
        }
        else {
          FUN_00467750(G.DAT_006d1da0, local_8[0x118], 4);
        }
      }
      else {
        FUN_00467750(G.DAT_006d1da0, local_8[0x118], 8);
      }
      FUN_00526ca0(local_8[0x1f0], 0);
      FUN_00526ca0(local_8[500], 1);
    }
    else if (local_8[0x234] !== 0) {
      local_8[0x1ec] = 1;
      local_8[0x1f8] = G.DAT_0062d7d0[local_8[0x1ec] * 2];
      local_8[0x1f0] = local_8[0x1f8];
      local_8[0x1fc] = G.DAT_0062d7d4[local_8[0x1ec] * 2];
      local_8[500] = local_8[0x1fc];
      FUN_00526ca0(local_8[0x1f0], 0);
      FUN_00526ca0(local_8[500], 1);
      FUN_00453c40();
    }
    break;
  }
  return;
}



// ============================================================
// Function: FUN_0052e326 @ 0x0052E326
// Size: 381 bytes
// ============================================================

// parley_go_back
export function FUN_0052e326() {
  let local_8;

  local_8 = FUN_005c62ee();
  if (local_8 === 0) {
    local_8 = 0;
  }
  else {
    local_8 = local_8 + -0x48;
  }
  FUN_004518d0();
  switch (local_8[0x1ec]) {
  case 3:
    break;
  case 5:
  case 6:
  case 7:
  case 8:
  case 9:
  case 10:
  case 0xb:
  case 0xc:
  case 0xd:
  case 0xf:
    local_8[0x1ec] = 3;
    FUN_00453c40();
    local_8[0x1f8] = G.DAT_0062d7e8;
    local_8[0x1f0] = local_8[0x1f8];
    local_8[0x1fc] = G.DAT_0062d7ec;
    local_8[500] = local_8[0x1fc];
    FUN_00526ca0(local_8[0x1f0], 0);
    FUN_00526ca0(local_8[500], 1);
    break;
  case 0xe:
    local_8[0x1ec] = 5;
    local_8[0x1f8] = G.DAT_0062d7f8;
    local_8[0x1f0] = local_8[0x1f8];
    local_8[0x1fc] = G.DAT_0062d7fc;
    local_8[500] = local_8[0x1fc];
    FUN_00526ca0(local_8[0x1f0], 0);
    FUN_00526ca0(local_8[500], 1);
    break;
  }
  return;
}



// ============================================================
// Function: FUN_0052e4c9 @ 0x0052E4C9
// Size: 179 bytes
// ============================================================

// parley_on_accept_deal
export function FUN_0052e4c9() {
  let local_8;

  local_8 = FUN_005c62ee();
  if (local_8 === 0) {
    local_8 = 0;
  }
  else {
    local_8 = local_8 + -0x48;
  }
  FUN_004518d0();
  FUN_0046b14d(0x83,
               G.DAT_006ad30c[G.DAT_006ad558[G.DAT_0067a8c0 * 4] * 0x54],
               0, 0, 0, 0, 0, 0, 0, 0);
  G.DAT_00635a3c = LAB_00403c74;
  FUN_00410030(s_PARLEYACCEPT2_006324f0, G.DAT_0063fc58, 0);
  local_8[0x1ec] = 3;
  FUN_004b8676(1);
  return;
}



// ============================================================
// Function: FUN_0052e57c @ 0x0052E57C
// Size: 265 bytes
// ============================================================

// parley_on_reject_deal
export function FUN_0052e57c() {
  let local_8;

  local_8 = FUN_005c62ee();
  if (local_8 === 0) {
    local_8 = 0;
  }
  else {
    local_8 = local_8 + -0x48;
  }
  FUN_004518d0();
  FUN_0046b14d(0x84,
               G.DAT_006ad30c[G.DAT_006ad558[G.DAT_0067a8c0 * 4] * 0x54],
               0, 0, 0, 0, 0, 0, 0, 0);
  local_8[0x1ec] = 2;
  local_8[0x1f8] = G.DAT_0062d7d0[local_8[0x1ec] * 2];
  local_8[0x1f0] = local_8[0x1f8];
  local_8[0x1fc] = G.DAT_0062d7d4[local_8[0x1ec] * 2];
  local_8[500] = local_8[0x1fc];
  FUN_00526ca0(local_8[0x1f0], 0);
  FUN_00526ca0(local_8[500], 1);
  return;
}



// ============================================================
// Function: FUN_0052e685 @ 0x0052E685
// Size: 149 bytes
// ============================================================

// parley_on_end_negotiations
export function FUN_0052e685() {
  let local_8;

  local_8 = FUN_005c62ee();
  if (local_8 === 0) {
    local_8 = 0;
  }
  else {
    local_8 = local_8 + -0x48;
  }
  FUN_004518d0();
  FUN_0046b14d(0x85,
               G.DAT_006ad30c[G.DAT_006ad558[G.DAT_0067a8c0 * 4] * 0x54],
               0, 0, 0, 0, 0, 0, 0, 0);
  local_8[0x1ec] = 3;
  FUN_004b8676(1);
  return;
}



// ============================================================
// Function: FUN_0052e71a @ 0x0052E71A
// Size: 152 bytes
// ============================================================

// parley_on_civ_button_click
export function FUN_0052e71a(param_1) {
  let iVar1;

  FUN_005c62ee();
  FUN_004518d0();
  if ((0x428 < param_1) && (param_1 < 0x430)) {
    iVar1 = ios_width(G.DAT_0068aeb0[(param_1 + -0x428) * 4]);
    G.DAT_0068af08[(param_1 + -0x428) * 4] = iVar1;
  }
  return;
}



// ============================================================
// Function: FUN_0052e7b7 @ 0x0052E7B7
// Size: 389 bytes
// ============================================================

// parley_on_checkbox_state_change
export function FUN_0052e7b7(param_1) {
  let iVar1;
  let local_c;

  FUN_005c62ee();
  FUN_004518d0();
  switch (param_1) {
  case 0x415:
    G.DAT_0068aedc = ios_width(G.DAT_0068aed0);
    break;
  case 0x416:
    G.DAT_0068aee4 = ios_width(G.DAT_0068aed8);
    if (G.DAT_0068aee4 !== 0) {
      for (local_c = 1; local_c < 8; local_c = local_c + 1) {
        if (G.DAT_0068ae90[local_c * 4] !== 0) {
          FUN_0040fad0(1);
          FUN_0040f380();
          G.DAT_0068aee8[local_c * 4] = 1;
        }
      }
    }
    break;
  case 0x417:
  case 0x418:
  case 0x419:
  case 0x41a:
  case 0x41b:
  case 0x41c:
  case 0x41d:
    param_1 = param_1 + -0x416;
    iVar1 = ios_width(G.DAT_0068ae90[param_1 * 4]);
    G.DAT_0068aee8[param_1 * 4] = iVar1;
    if ((G.DAT_0068aee8[param_1 * 4] === 0) && (G.DAT_0068aee4 !== 0)) {
      FUN_0040fad0(0);
      FUN_0040f380();
      G.DAT_0068aee4 = 0;
    }
    break;
  case 0x430:
    G.DAT_0068aee0 = ios_width(G.DAT_0068aed4);
    break;
  }
  return;
}



// ============================================================
// Function: FUN_0052e971 @ 0x0052E971
// Size: 678 bytes
// ============================================================

// parley_update_button_states
export function FUN_0052e971(in_ECX) {
  if ((in_ECX[0x348] !== 0) && (in_ECX[0x34c] !== 0)) {
    if (in_ECX[0x1ec] !== 2) {
      FUN_0043c5f0();
      FUN_00453c40();
      FUN_0043c5f0();
      FUN_00453c40();
    }
    switch (in_ECX[0x1ec]) {
    case 1:
    case 2:
      FUN_0043c5f0();
      FUN_00453c40();
      FUN_0040f380();
      FUN_00453c40();
      if (in_ECX[0x1ec] === 2) {
        if ((s8(in_ECX[0x3bd]) === 0) && (s8(in_ECX[0x3be]) === 0)) {
          FUN_0043c5f0();
          FUN_00453c40();
          FUN_0040f380();
          FUN_00453c80();
        }
        else {
          FUN_0040f380();
          FUN_00453c80();
          FUN_0043c5f0();
          FUN_00453c40();
        }
      }
      break;
    case 3:
    case 5:
      FUN_0043c5f0();
      FUN_00453c40();
      FUN_0040f380();
      FUN_00453c80();
      break;
    case 6:
    case 7:
    case 8:
    case 9:
    case 10:
    case 0xb:
    case 0xc:
    case 0xd:
    case 0xf:
      FUN_0040f380();
      FUN_0043c5f0();
      FUN_00453c40();
      if (in_ECX[0x1e8] === 0) {
        FUN_00453c40();
      }
      else {
        FUN_00453c80();
      }
      break;
    case 0xe:
      FUN_0040f380();
      FUN_0043c5f0();
      FUN_00453c40();
      if ((in_ECX[0x1e4] === 0) || (in_ECX[0x1e8] === 0)) {
        FUN_00453c40();
      }
      else {
        FUN_00453c80();
      }
      break;
    }
  }
  return;
}



// ============================================================
// Function: FUN_0052ec47 @ 0x0052EC47
// Size: 329 bytes
// ============================================================

// find_unit_by_alive_flag
export function FUN_0052ec47(param_1) {
  let cVar1;
  let sVar2;
  let sVar3;
  let bVar4;
  let uVar5;
  let iVar6;
  let local_20;
  let local_8;

  local_20 = G.DAT_00655b16 | 0;
  do {
    local_20 = local_20 + -1;
    if (local_20 < 0) {
      return -1;
    }
  } while ((G.DAT_006560f0[local_20 * 0x20 + 0x1a] === 0 &&
            G.DAT_006560f0[local_20 * 0x20 + 0x1b] === 0 &&
            G.DAT_006560f0[local_20 * 0x20 + 0x1c] === 0 &&
            G.DAT_006560f0[local_20 * 0x20 + 0x1d] === 0) ||
           ((G.DAT_006560f0[local_20 * 0x20 + 0x1a] |
             (G.DAT_006560f0[local_20 * 0x20 + 0x1b] << 8) |
             (G.DAT_006560f0[local_20 * 0x20 + 0x1c] << 16) |
             (G.DAT_006560f0[local_20 * 0x20 + 0x1d] << 24)) !== param_1));
  // Unit x (int16 at offset 0x00)
  sVar2 = (G.DAT_006560f0[local_20 * 0x20 + 0] | (G.DAT_006560f0[local_20 * 0x20 + 1] << 8));
  if (sVar2 & 0x8000) sVar2 = sVar2 | 0xFFFF0000;
  // Unit y (int16 at offset 0x02)
  sVar3 = (G.DAT_006560f0[local_20 * 0x20 + 2] | (G.DAT_006560f0[local_20 * 0x20 + 3] << 8));
  if (sVar3 & 0x8000) sVar3 = sVar3 | 0xFFFF0000;
  bVar4 = false;
  local_8 = 0;
  do {
    if (7 < local_8) {
      // LAB_0052ed47
      if ((!bVar4) && (iVar6 = FUN_005b50ad(local_20, 2), iVar6 !== 1)) {
        local_20 = -2;
      }
      return local_20;
    }
    uVar5 = FUN_005ae052(s8(G.DAT_00628350[local_8]) + sVar2);
    cVar1 = s8(G.DAT_00628360[local_8]);
    iVar6 = FUN_004087c0(uVar5, cVar1 + sVar3);
    if ((iVar6 !== 0) && (iVar6 = FUN_005b89e4(uVar5, cVar1 + sVar3), iVar6 === 0)) {
      bVar4 = true;
      // goto LAB_0052ed47 — restructured: jump to top of loop check
      if ((!bVar4) && (iVar6 = FUN_005b50ad(local_20, 2), iVar6 !== 1)) {
        local_20 = -2;
      }
      return local_20;
    }
    local_8 = local_8 + 1;
  } while (true);
}



// ============================================================
// Function: FUN_0052ed95 @ 0x0052ED95
// Size: 128 bytes
// ============================================================

// find_city_by_id
export function FUN_0052ed95(param_1) {
  let local_8;

  local_8 = G.DAT_00655b18 | 0;
  do {
    local_8 = local_8 + -1;
    if (local_8 < 0) {
      return -1;
    }
  } while ((G.DAT_0064f394[local_8 * 0x58] === 0) ||
           (G.DAT_0064f394[local_8 * 0x58] !== param_1));
  return local_8;
}
