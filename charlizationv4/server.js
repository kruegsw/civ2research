...
import { generateDiplomacyActions } from "../charlizationv3/engine/ai/diplomai.js";
...
// Initialize v4 engine at startup. RULES.TXT lives at the canonical tracked
// location; CIV2_RULES env var overrides for ad-hoc testing.
const RULES_PATH = process.env.CIV2_RULES
  || path.join(__dirname_early, '..', 'reverse_engineering', 'findings', 'RULES.TXT');
try {
  initV4(fs.readFileSync(RULES_PATH, 'utf8'));
  console.log(`[v4] Binary engine initialized from ${RULES_PATH}`);
} catch (e) {
  console.error(`[v4] Init failed (${RULES_PATH}): ${e.message} — v4EndTurn will no-op`);
}
...
const PUBLIC_DIR = path.join(__dirname, "..", "charlizationv3", "public");
const ENGINE_DIR = path.join(__dirname, "..", "charlizationv3", "engine");
  ...
  ws.on("message", async (raw) => {
    ...
    switch (msg.type) {
        ...
        if (allReady && !room.countdownTimer) {
          ...
          room.countdownTimer = setInterval(async () => {
            ...
            if (room.countdownSeconds > 0) {
              ...
              // Initialize game
              await startGame(roomId, room, finalOccupied);
              broadcastRoomList();
            }
          }, 1000);
      ...
      case "ACTION": {
        ...
        // Run v4 binary engine for END_TURN to get faithful yields
        if (msg.action.type === 'END_TURN') {
          try {
            next = await v4EndTurn(next, civSlot);
          } catch (e) {
            console.warn('[v4] END_TURN:', e.message?.substring(0, 80));
          }
        }
        ...
        try {
          ...
          if (msg.action.type === 'END_TURN') {
            await processAiTurns(actionRoomId, room);
          }
        } catch (postActionErr) {
      ...
      case "RESTART_GAME": {
        ...
        await processAiTurns(restartRoomId, restartRoom);
 ...
 */
async function processAiTurns(roomId, room) {
  const humanPlayers = room.gameState.humanPlayers || 0;
  ...
  for (let iter = 0; iter < MAX_ITERATIONS; iter++) {
    ...
    //
    // Phase 1: Strategy + batch actions (econ, diplomacy, production).
    //   These are civ-level decisions that don't depend on per-unit state.
    //   Generated against initial state, applied as a batch.
    //
    // Phase 2: Per-unit actions (settlers, military, cleanup).
    //   Binary processes units ONE AT A TIME — each unit's action is
    //   executed before the next unit decides, so later units see the
    //   updated state. We generate one action per unit against the
    //   CURRENT state, apply it, then generate the next.
    const wantDebug = roomHasDebugClient(room);
    const debugLog = wantDebug ? [] : null;
    const t0 = Date.now();

    // Phase 1: Strategy + batch actions
    let aiResult;
    try {
      ...
      aiResult = { actions: [], strategy: null, goals: null, debugLog: [] };
    }
    ...
    const t1 = Date.now();
    console.log(`[ai] Room ${roomId}: civ ${activeCiv} (${room.gameState.civNames?.[activeCiv] || '?'}) — ${batchActions.length} batch actions in ${t1 - t0}ms`);
    ...
    // Apply actions through the reducer.
    // Binary FUN_0053184D: each unit action is applied before the next
    // unit decides. Batch actions (econ/diplo/production) are applied
    // first, then unit actions one at a time.
    if (!room.aiCombatQueue) room.aiCombatQueue = [];
    if (!room.aiMoveQueue) room.aiMoveQueue = [];
    const t2 = Date.now();
    ...
    // Apply batch actions first (econ/diplo/production)
    for (const action of batchActions) {
      let result;
      try {
        result = applyAction(room.gameState, room.mapBase, action, activeCiv);
      } catch (err) {
        console.error(`[CRASH] AI batch applyAction threw for civ ${activeCiv}, action ${action.type}:`, err);
        continue;
      }
      if (result !== room.gameState) {
        room.gameState = result;
    ...
    try {
      ...
      // Run v4 binary for faithful yields
      endResult = await v4EndTurn(endResult, activeCiv);
    } catch (err) {
    ...
    if (endResult === room.gameState) {
      ...
      endResult = applyAction(room.gameState, room.mapBase, { type: 'END_TURN' }, activeCiv);
      try { endResult = await v4EndTurn(endResult, activeCiv); } catch(e) {}
    }
...
async function startGame(roomId, room, occupiedSeats) {
  ...
  // Process AI turns if the initial activeCiv is AI-controlled
  await processAiTurns(roomId, room);
