...
const replayYieldsFlag = args.includes('--replay-yields') ? ['--replay-yields'] : [];
const replayTreasuryFlag = args.includes('--replay-treasury') ? ['--replay-treasury'] : [];
...
const v4Out = run('node', [join(__dirname, 'dump-server-state.js'), savPath,
  ...
  ...replayYieldsFlag, ...replayTreasuryFlag]);
