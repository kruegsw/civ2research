...
| File | Size | Map | Cities | Description |
|------|------|-----|--------|-------------|
| `123456 Claire's save.sav` | 72,643 bytes | 44×63 | 17 active | Mid-game save. 5 civilizations with cities. |
| `St_Auto.SAV` | 50,787 bytes | 44×63 | 11 active | Autosave file. Late game, heavy conquest. Player "Stubear" as Vikings. |
| `20260123_stubear.sav` | 51,235 bytes | 44×63 | 11 active | Late game. Player "Stubear" as Americans. Dominant power controlling ~95% of the map. |
...
### Header (Bytes 0x0000–0x0011)

| Offset | Size | Field | Example |
|--------|------|-------|---------|
| 0x0000 | 8 bytes | Magic signature | `CIVILIZE` (ASCII) |
| 0x0008 | 1 byte | Separator | `0x1A` |
| 0x0009 | 1 byte | Version/format | `0x2C` (= 44, MGE format) |
| 0x000A | 2 bytes | Map width (LE uint16) | `44` |
| 0x000C | 2 bytes | Map height (LE uint16) | `63` |
...
Found at offset `0x0612` as a null-terminated ASCII string (e.g., `"Stubear"`). Residual bytes from previous longer names may follow the null terminator (e.g., `"oln"` from overwriting `"Lincoln"`).
...
**Finding the tile data offset**: The offset varies between saves. Search for a contiguous block of `num_tiles × 6` bytes where byte[0] & 0x0F is always ≤ 10 (valid terrain) and the terrain distribution includes ocean (type 10) at a realistic proportion (typically 40–60% of tiles).

In saves analyzed:
- Claire's save: tiles at `0x0A2F4`
- St_Auto.SAV: tiles at `0x06bc0`
- stubear.sav: tiles at `0x076dc`
...
| 5 | Continent / region | Appears to encode continent ID or territory region. Values like `0xF0`, `0x50`, `0x30`, `0x20` are common. |
...
The minimap colors each tile by the **highest bit set** — i.e., the last civ to explore it. In the `stubear.sav` game:
- 692 of 726 land tiles (95%) have only bit 0 set → Americans (slot 0) control nearly everything
- 31 tiles have bit 2 set → a minor civ
- 3 tiles have bit 3 set → another minor civ
...
City records are **88 bytes each**, stored contiguously. The block begins after the tile data (with possible intervening data). Find it by searching for known capital city names (`Washington`, `Rome`, `London`, `Delhi`, `Kyoto`, etc.).
...
Notes:
- Cities with `size = 0` are destroyed or disbanded. They remain in the save as historical records.
...
Found after the city records, as a null-terminated ASCII string. In the saves analyzed, only the **player's tribe name** was found (e.g., `"Vikings"` at `0xC543`, `"Aztecs"` at `0xC703`). AI tribe names may be stored differently or computed from `LEADERS.TXT` at runtime.
