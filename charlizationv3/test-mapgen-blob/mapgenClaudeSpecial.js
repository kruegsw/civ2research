// ═══════════════════════════════════════════════════════════════════
// mapgenClaudeSpecial.js — Realistic satellite-view map generation
//
// 13-phase pipeline designed for natural-looking planet maps:
...
//   5. Mountain chains (interior + coastal) with flanking hills
//   6. Terrain assignment:
//      a. Latitude bands (equator=jungle, 30°=desert, 60°=boreal)
...
//                   jungle/swamp extent, river count
//   age          → mountain count/length, coastline roughness,
//                   smoothing intensity, hill density, peninsula/bay count
// ═══════════════════════════════════════════════════════════════════
 ...
 */
export function generateMapClaudeSpecial(settings = {}) {
  ...
  for (let dy = 0; dy < mh; dy++) {
    for (let dx = 0; dx < dw; dx++) {
      ...
      const jitter = (rng.random() - 0.5) * 0.07;
      const eLat = Math.max(0, Math.min(1, lat - shift + jitter));
  ...
  // ══════════════════════════════════════════════════════════════
  // Phase 7: Smoothing (light neighbor voting)
