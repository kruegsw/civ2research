// ═══════════════════════════════════════════════════════════════════
// mapgenClaudeSpecial4.js — Realistic satellite-view map generation (v4)
//
// v4: Continental banana bending. After the land mask is fully shaped
// (coastlines, peninsulas, bays, seas, islands, fjords), each continent
// is curved into a crescent/banana arc via inverse-mapped parabolic
// warp. Preserves all coastline detail while adding organic curvature.
//
...
//   3h. Continental banana bending (parabolic warp per landmass)
 ...
 */
export function generateMapClaudeSpecial4(settings = {}) {
  ...
  // ══════════════════════════════════════════════════════════════
  // Phase 3h: Continental banana bending
  // ══════════════════════════════════════════════════════════════
  // Each continent is curved into a crescent/banana arc using a
  // parabolic warp. The transform:
  //   - Find principal axis (longer of width vs height)
  //   - For each tile, project onto axis to get t ∈ [-1, 1]
  //   - Displace perpendicular by bendAmount * t² (parabola)
  // Uses inverse mapping to preserve all coastline detail without gaps.
  {
    // Flood-fill to identify continent bodies in display space
    const bodyMap = new Int16Array(dw * mh).fill(-1);
    const bodyList = [];

    for (let i = 0; i < dw * mh; i++) {
      if (!land[i] || bodyMap[i] >= 0) continue;
      const bid = bodyList.length;
      const b = { tiles: [], minY: mh, maxY: 0, sinSum: 0, cosSum: 0, sumY: 0 };
      const queue = [i];
      bodyMap[i] = bid;

      while (queue.length > 0) {
        const ci = queue.shift();
        const bx = ci % dw, by = (ci / dw) | 0;
        b.tiles.push(ci);
        // Circular mean for x (handles wrapping correctly)
        const angle = (2 * Math.PI * bx) / dw;
        b.sinSum += Math.sin(angle);
        b.cosSum += Math.cos(angle);
        b.sumY += by;
        if (by < b.minY) b.minY = by;
        if (by > b.maxY) b.maxY = by;

        for (const [ndx, ndy] of N4) {
          const nx = wX(bx + ndx), ny = by + ndy;
          if (oob(nx, ny)) continue;
          const ni = ny * dw + nx;
          if (land[ni] && bodyMap[ni] < 0) {
            bodyMap[ni] = bid;
            queue.push(ni);
          }
        }
      ...
      // Circular mean gives correct centroid even for wrapping bodies
      let cx = (Math.atan2(b.sinSum, b.cosSum) / (2 * Math.PI)) * dw;
      if (cx < 0) cx += dw;
      b.cx = cx;
      b.cy = b.sumY / b.tiles.length;

      // Effective x-span via wrapped distance from centroid
      let maxDistX = 0;
      for (const ti of b.tiles) {
        const bx = ti % dw;
        let dx = Math.abs(bx - cx);
        if (wraps && dx > dw / 2) dx = dw - dx;
        if (dx > maxDistX) maxDistX = dx;
      }
      b.halfSpanX = maxDistX;
      b.spanY = b.maxY - b.minY;

      bodyList.push(b);
    }

    // Compute bend parameters per body
    const MIN_BEND_SIZE = 40;
    const bendParams = bodyList.map(b => {
      if (b.tiles.length < MIN_BEND_SIZE) return null;

      const spanX = b.halfSpanX * 2;
      const spanY = b.spanY;

      const horizontal = spanX >= spanY;
      const axisLen = horizontal ? spanX : spanY;
      if (axisLen < 8) return null;

      // Bend strength: 18-28% of axis length
      const bendAmount = axisLen * (0.18 + rng.random() * 0.10);
      const bendDir = rng.random() < 0.5 ? 1 : -1;
      const halfAxis = axisLen / 2;

      // Clamp bend if it would push edges off the map vertically
      let clampedBend = bendAmount;
      if (horizontal) {
        if (b.cy + clampedBend > mh - 3 || b.cy - clampedBend < 2)
          clampedBend *= 0.5;
      }

      return { horizontal, bendAmount: clampedBend, bendDir, cx: b.cx, cy: b.cy, halfAxis };
    });

    // Inverse mapping: for each output (nx, ny), compute original position
    // via inverse bend for each body, check if it was land in that body.
    const newLand = new Uint8Array(dw * mh);

    // Helper: wrapped distance from a to b
    function wDist(a, b) {
      let d = a - b;
      if (wraps) {
        if (d > dw / 2) d -= dw;
    ...
    for (let ny = 0; ny < mh; ny++) {
      for (let nx = 0; nx < dw; nx++) {
        let found = false;

        for (let bi = 0; bi < bodyList.length; bi++) {
          const bp = bendParams[bi];

          if (bp === null) {
            // Small body / skipped: keep at original position
            if (bodyMap[ny * dw + nx] === bi) { found = true; break; }
            continue;
          }

          // Inverse: undo the bend to find original position
          let ox, oy;
          if (bp.horizontal) {
            ox = nx;
            // Use wrapped distance for t computation
            const dx = wDist(nx, bp.cx);
            const t = Math.max(-1, Math.min(1, dx / bp.halfAxis));
            oy = Math.round(ny - bp.bendDir * bp.bendAmount * t * t);
          } else {
            oy = ny;
            const t = Math.max(-1, Math.min(1, (ny - bp.cy) / bp.halfAxis));
            ox = wX(Math.round(nx - bp.bendDir * bp.bendAmount * t * t));
          }

          if (oy >= 0 && oy < mh) {
            const oxi = wraps ? wX(ox) : ox;
            if (oxi >= 0 && oxi < dw && bodyMap[oy * dw + oxi] === bi) {
              found = true; break;
            }
          }
        }

        if (found) newLand[ny * dw + nx] = 1;
      }
    }

    // Replace land array with bent version
    for (let i = 0; i < dw * mh; i++) land[i] = newLand[i];
  }
