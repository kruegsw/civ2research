...
//
// Depends on globals from app.js (loaded first):
//   viewportCanvas, vpX, vpY, vpScale, offW, offH, wrapW, wraps,
//   vpLogicalW, clampViewport, drawViewport, resizeViewport,
//   currentMapData, handleMapClick, closeCityDialog, closeCityView
// ═══════════════════════════════════════════════════════════════════
...
viewportCanvas.addEventListener('touchstart', e => {
...
viewportCanvas.addEventListener('touchmove', e => {
  if (touchMode === 'pinch' && e.touches.length === 2) {
    ...
    // Map point under previous pinch center
    const prevLocalX = pinchLastCX - rect.left;
    const prevLocalY = pinchLastCY - rect.top;
    const mapX = vpX + prevLocalX / vpScale;
    const mapY = vpY + prevLocalY / vpScale;

    // Apply new scale
    vpScale = Math.max(VP_MIN_SCALE, Math.min(VP_MAX_SCALE,
      pinchStartScale * dist / pinchStartDist));

    // Keep same map point under new pinch center
    const newLocalX = cx - rect.left;
    const newLocalY = cy - rect.top;
    vpX = mapX - newLocalX / vpScale;
    vpY = mapY - newLocalY / vpScale;
...
viewportCanvas.addEventListener('touchend', e => {
  if (e.touches.length === 0) {
    if (touchMode === 'drag') {
      ...
      if (dist < 10) {
        handleMapClick({ clientX: dragLastX, clientY: dragLastY, target: viewportCanvas });
      }
    }
    touchMode = 'none';
  } else if (e.touches.length === 1 && touchMode === 'pinch') {
    // Went from 2 fingers to 1 — stop pinch, wait for fresh touchstart
    touchMode = 'none';
  }
}, { passive: true });

// ── Trackpad / mouse wheel: pan + pinch-to-zoom ──
viewportCanvas.addEventListener('wheel', e => {
  if (offW === 0) return;
  e.preventDefault();
  if (e.ctrlKey) {
    // Trackpad pinch gesture (browser sends ctrlKey + deltaY for pinch)
    const rect = viewportCanvas.getBoundingClientRect();
    const localX = e.clientX - rect.left;
    const localY = e.clientY - rect.top;
    const mapX = vpX + localX / vpScale;
    const mapY = vpY + localY / vpScale;

    const zoomFactor = 1 - e.deltaY * 0.01;
    vpScale = Math.max(VP_MIN_SCALE, Math.min(VP_MAX_SCALE, vpScale * zoomFactor));

    vpX = mapX - localX / vpScale;
    vpY = mapY - localY / vpScale;
  } else {
    // Two-finger swipe = pan
    vpX += e.deltaX / vpScale;
    vpY += e.deltaY / vpScale;
  }
  clampViewport();
  drawViewport();
}, { passive: false });
